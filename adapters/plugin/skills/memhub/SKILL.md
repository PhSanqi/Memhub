...
- `memmy_context` recalls relevant L4 account context, L2/L3 project context, and a small set of explicitly reusable Skill candidates. Retrieval v1 already applies legal-scope filtering, semantic + lexical reranking, deduplication/diversity, and a small Top-K. Use its retrieval diagnostics when recall quality needs inspection; do not guess a project merely to broaden recall.
- Skill candidates from `memmy_context` are summaries, not the full procedure. Load a selected candidate with `memhub_skill action=load skill_id=...`; use the returned `execution_id` to record `invoked` and then `success`/`failure`, plus `user_correction` when applicable. Do not claim a Skill was executed merely because it was retrieved or loaded.
