...
function str(description) { return { type: 'string', description, required: true }; }
function strOpt(description) { return { type: 'string', description, required: false }; }
function numOpt(description) { return { type: 'number', description, required: false }; }
function boolOpt(description) { return { type: 'boolean', description, required: false }; }
/**
...
function toJsonSchema(node) {
    ...
    for (const [key, value] of Object.entries(source)) {
        if (key === 'required') {
            // 已编译过的对象级 required（string[]）：保留并合并，保证 toJsonSchema 幂等。
            // 否则 `params({ frontmatter: moduleParams() })` 这类"编译后再嵌套"会二次编译，
            // 把内层必填表整段丢掉（0.5.1 及以前 normify_module_upsert.parameters.required 丢失）。
            ...
            continue; // 属性级内联 required: true 由上面的 properties 遍历重新派生
        }
...
function params(props, _required = []) {
    ...
    return toJsonSchema({ type: 'object', properties: props });
}
...
export function createNormifyTools(env) {
    const toolCatalog = [];
    const registered = [];
    ...
    const resolve = async (args, create = false) => {
        const hasProject = typeof args.project === 'string' && args.project.trim() !== '';
        const hasDir = typeof args.dir === 'string' && args.dir.trim() !== '';
        if (!hasProject && !hasDir) {
            if (env.currentProject !== undefined)
                return resolveProject(env.rootDir, { project: env.currentProject }, { create, accountId: env.accountId });
            const projects = listProjects(env.rootDir, { accountId: env.accountId });
            if (projects.length === 1) {
                env.currentProject = projects[0].slug;
                return projects[0];
            }
        }
        return resolveProject(env.rootDir, { project: args.project, dir: args.dir }, { create, accountId: env.accountId });
    };
    register('normify_tree_list', {
        ...
        const projects = listProjects(rootDir, { accountId: env.accountId });
        if (env.currentProject === undefined && projects.length === 1)
            env.currentProject = projects[0].slug;
        const out = [];
        ...
        return { ok: true, rootDir: env.rootDir, ...(env.accountId === undefined ? {} : { account_scoped: true }), current_project: env.currentProject ?? null, projects: out };
    });
    ...
    register('normify_sync', {
        ...
        try {
            const tree = JSON.parse(await readFile(join(proj.dir, 'tree.json'), 'utf8'));
    ...
    register('normify_layout_upsert', {
        ...
        const data = { schema_version: LAYOUT_SCHEMA_VERSION, id, updated_at: new Date().toISOString() };
    ...
    register('normify_brief', {
        ...
        const impact = { direct: [], transitive: [], cross_tree: [] };
        ...
        for (let d = 1; d <= depth; d++) {
            ...
            for (const id of frontier) {
                for (const ref of depsIn.get(id) ?? []) {
                    ...
                    const tree = (candidate) => candidate.split('.')[0];
    ...
    register('normify_change_list', {
        ...
        const counts = { proposed: 0, in_progress: 0, verified: 0, abandoned: 0 };
        for (const c of out)
