...
function accountRoot(rootDir, accountId) {
    const root = resolve(rootDir);
    if (accountId === undefined)
        return root;
    const account = accountId.trim();
    if (account === '')
        throw new NormifyError('account/invalid', 'accountId 不能为空');
    const key = createHash('sha256').update(account, 'utf8').digest('hex');
    return join(root, '.normify', 'accounts', key);
}
function assertInsideAccount(root, candidate) {
    const rel = relative(root, candidate);
    if (rel === '..' || rel.startsWith('..' + sep))
        throw new NormifyError('project/outside-account', '结构数据目录不属于当前账号空间：' + candidate);
    const realRoot = realpathSync(root);
    const realCandidate = realpathSync(candidate);
    const realRel = relative(realRoot, realCandidate);
    if (realRel === '..' || realRel.startsWith('..' + sep))
        throw new NormifyError('project/outside-account', '结构数据目录解析后不属于当前账号空间：' + candidate);
}
...
export async function resolveProject(rootDir, args, opts = {}) {
    const root = accountRoot(rootDir, opts.accountId);
...
export function listProjects(rootDir, opts = {}) {
    const root = accountRoot(rootDir, opts.accountId);
    let entries = [];
    try {
