import { compactAnalyticsParams, createCliAnalytics, elapsedMs, errorCodeFromUnknown, resolveAnalyticsAppEnv, resolveAnalyticsDebugMode, } from "../cli/analytics.js";
...
export function resolveExternalAgentSource(input) {
 ...
 */
export function resolveExternalPluginToolCallSource(input) {
    const adapterId = stringValue(input.adapterId);
    if (adapterId?.startsWith("agent-source:"))
        return null;
    if (adapterId) {
        const pluginMatch = /^memmy-(.+)-plugin$/u.exec(adapterId);
...
export function resolvePluginTarget(agentSourceId) {
    if (HOOK_AGENT_SOURCE_IDS.has(agentSourceId))
        return "hook";
    if (NATIVE_PLUGIN_AGENT_SOURCE_IDS.has(agentSourceId))
        return "native_plugin";
    return undefined;
}
export function buildPluginRuntimeBaseParams(agentSourceId, input, hookNameDefault) {
...
export function resolveHookName(value, fallback) {
    const trimmed = stringValue(value);
    return trimmed ?? fallback;
}
export function resolveCaptureFallbackMode(turnId) {
    const normalized = stringValue(turnId);
    if (normalized?.includes("-fallback-"))
        return "synthetic_turn_id";
    return undefined;
}
export function hitCountFromTurnStartResponse(response) {
    if (!response || typeof response !== "object")
        return 0;
    const ids = response.sourceMemoryIds;
...
export function hasInjectedContextFromTurnStartResponse(response) {
    if (!response || typeof response !== "object")
        return false;
    const injected = response.injectedContext;
    if (typeof injected === "string")
        return Boolean(injected.trim());
    if (!injected || typeof injected !== "object")
        return false;
    const markdown = injected.markdown;
    return typeof markdown === "string" && Boolean(markdown.trim());
}
export function storedCountFromCompleteTurnResponse(response) {
    if (!response || typeof response !== "object")
        return 0;
...
export function toolCallCountFromCompleteTurnRequest(request) {
    if (!request || typeof request !== "object")
        return undefined;
    const toolCalls = request.toolCalls;
    return Array.isArray(toolCalls) ? toolCalls.length : undefined;
}
export function hitCountFromSearchResponse(response) {
    if (!response || typeof response !== "object")
        return 0;
    const record = response;
...
export function hitCountFromGetResponse(response) {
    if (!response || typeof response !== "object")
        return 0;
    const id = response.id;
    return typeof id === "string" && id.trim() ? 1 : 0;
}
export function storedCountFromAddResponse(response) {
    if (!response || typeof response !== "object")
        return 0;
    const id = response.id ?? response.memoryId;
    return typeof id === "string" && id.trim() ? 1 : 0;
}
export async function trackExternalHookRecall(analytics, input, run) {
...
export async function trackExternalHookCapture(analytics, input, request, run) {
    const agentSourceId = resolveExternalAgentSource(input);
    if (!agentSourceId)
        return await run();
...
export async function trackExternalToolCall(analytics, input, run, mapSuccess) {
