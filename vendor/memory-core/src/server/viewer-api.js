...
export function isViewerApiRequest(request, url) {
    return url.pathname === "/api/v1/events" || header(request, "x-memmy-viewer") === "1";
}
export function assertLocalViewerRequest(request, url) {
...
export async function routeViewerRequest(context, method, url, body) {
    ...
    if ((method === "POST" || method === "DELETE") && sourceConnection?.[1] && sourceConnection[2]) {
        return {
            body: await context.agentSources.mutateConnection(decodeURIComponent(sourceConnection[1]), sourceConnection[2], method)
        };
    }
    ...
    if (method === "POST" && path === "/api/v1/traces/delete") {
        ...
        for (const id of ids)
            context.service.deleteMemory(id, envelope);
...
export function streamViewerEvents(context, request, response, url) {
...
async function viewerConfig(context) {
...
async function patchViewerConfig(context, body) {
...
async function rawMemoryConfig(configPath) {
    if (!configPath)
        return {};
    try {
...
function stripMaskedSecrets(value) {
    ...
    for (const [key, item] of Object.entries(value)) {
        if (/token|apiKey|secret|password/i.test(key) && (item === "********" || item === "[redacted]"))
            continue;
