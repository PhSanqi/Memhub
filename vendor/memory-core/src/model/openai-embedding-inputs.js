...
let encoder;
export function planOpenAiEmbeddingInputs(texts, model, configuredMaxInputTokens) {
    ...
    if (totalTokens <= OPENAI_EMBEDDING_BATCH_TOKEN_BUDGET &&
        encoded.every((tokens) => tokens.length <= inputTokenBudget))
        return null;
    const chunks = encoded.flatMap((tokens, originalIndex) => {
        if (tokens.length === 0)
            return [{ originalIndex, tokens, input: useTokenIds ? tokens : "" }];
        ...
        for (let offset = 0; offset < tokens.length;) {
            ...
            if (!useTokenIds && end < tokens.length) {
                ...
                if (end === offset)
                    end = Math.min(tokens.length, offset + inputTokenBudget);
            }
...
export function aggregateOpenAiEmbeddingVectors(plan, vectors) {
    ...
    return Array.from({ length: plan.originalCount }, (_item, originalIndex) => {
        ...
        const mean = Array.from({ length: dimensions }, (_value, dimension) => entries.reduce((sum, entry) => sum + entry.vector[dimension] * Math.max(1, entry.chunk.tokens.length), 0) / totalWeight);
        const norm = Math.hypot(...mean);
        return norm > 0 ? mean.map((value) => value / norm) : mean;
    });
}
function isKnownOpenAiEmbeddingModel(model) {
    return /(?:^|[/.:])text-embedding-(?:3-(?:small|large)|ada-002)(?:$|[/.:])/i.test(model?.trim() ?? "");
}
function resolveInputTokenBudget(_model, configured) {
...
function decodeTokenBytes(tokenBytes) {
    const bytes = tokenBytes.flatMap((value) => Array.from(value));
    return new TextDecoder().decode(Uint8Array.from(bytes));
}
function startsWithContinuationByte(bytes) {
    const first = bytes?.[0];
