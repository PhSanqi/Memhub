...
export function migrate(db) {
    ...
    if (version > SCHEMA_VERSION) {
        throw new Error(`Memory database schema version ${version} is newer than supported version ${SCHEMA_VERSION}`);
    }
    ...
    try {
        db.transaction(() => {
            ...
            db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS uq_evolution_jobs_active_dedupe
         ON evolution_jobs (dedupe_key)
         WHERE dedupe_key IS NOT NULL AND status IN ('queued', 'leased', 'failed')`).run();
            ...
            db.prepare(`INSERT INTO schema_migrations (id, version, applied_at, checksum)
         VALUES (?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET
           version = excluded.version,
           applied_at = excluded.applied_at,
           checksum = excluded.checksum`).run(SCHEMA_MIGRATION_ID, SCHEMA_VERSION, now, checksum);
        })();
    }
    finally {
...
function backfillMemoryCaptureClaims(db) {
    const rows = db.prepare(`SELECT raw_turns.user_id,
     ...
     ORDER BY raw_turns.created_at ASC, raw_turns.id ASC`).all();
    const insert = db.prepare(`INSERT OR IGNORE INTO memory_capture_claims (
       user_id, source, qa_hash, primary_memory_id, captured_by, created_at
     ) VALUES (?, ?, ?, ?, 'turn_complete', ?)`);
    for (const row of rows) {
        insert.run(row.user_id, normalizeMemoryCaptureSource(row.source), memoryCaptureQaHash(row.user_text, row.assistant_text), row.primary_memory_id, row.created_at);
    }
}
function migrateLegacyWorldModels(db, now) {
    db.prepare(`UPDATE evolution_jobs
     SET status = 'dead_letter',
         leased_until = NULL,
...
function backfillLegacyAdapterHostSessionKeys(db, now) {
    db.prepare(`UPDATE sessions AS candidate
...
function backfillMemoryProcessingState(db, now) {
    db.prepare(`INSERT INTO memory_processing_state (
     ...
     ON CONFLICT(memory_id) DO NOTHING`).run(now);
    db.prepare(`DELETE FROM evolution_jobs
     WHERE job_type IN ('trace_summary', 'import_summary', 'embedding')
       AND target_memory_id IN (SELECT memory_id FROM memory_processing_state)`).run();
    db.prepare(`DELETE FROM embedding_retry_queue
     WHERE target_kind = 'trace'
       AND target_id IN (SELECT memory_id FROM memory_processing_state)`).run();
}
function removeLegacyProcessingMetadata(db) {
    const rows = db.prepare(`SELECT id, tags_json, info_json, properties_json, memory_value, status, deleted_at
     FROM memories
     WHERE memory_layer = 'L1'
       AND json_extract(properties_json, '$.internal_info.memory_kind') = 'trace'`).all();
    for (const row of rows) {
        ...
        if (Array.isArray(info.tags))
            info.tags = stripProcessingTags(info.tags);
        if (Array.isArray(properties.tags))
            properties.tags = stripProcessingTags(properties.tags);
        const publicInfo = isRecord(properties.info) ? properties.info : undefined;
        if (publicInfo) {
            delete publicInfo.import_pipeline;
            if (Array.isArray(publicInfo.tags))
                publicInfo.tags = stripProcessingTags(publicInfo.tags);
        }
        const internalInfo = isRecord(properties.internal_info) ? properties.internal_info : undefined;
        if (internalInfo)
            delete internalInfo.import_pipeline;
        db.prepare(`UPDATE memories SET tags_json = ?, info_json = ?, properties_json = ? WHERE id = ?`).run(JSON.stringify(tags), JSON.stringify(info), JSON.stringify(properties), row.id);
        db.prepare(`DELETE FROM memories_fts WHERE id = ?`).run(row.id);
        if (!row.deleted_at && row.status !== "deleted") {
            db.prepare(`INSERT INTO memories_fts (id, identifier, memory_value, tags) VALUES (?, ?, ?, ?)`).run(row.id, row.id, row.memory_value, tags.join(" "));
        }
    }
}
...
function stripProcessingTags(values) {
