...
export class EpisodeReadModel {
    ...
    timeline(input) {
        ...
        const layers = input.layers && input.layers.length > 0 ? input.layers : ["L1"];
        let scopedEpisode;
        let scopedSession;
        if (input.sessionId) {
    ...
    timelineItemsFromEpisodes(episodes, layers, tags, limit, cursor) {
    ...
    getMemory(id, request = {}) {
        ...
        if (!memory)
            throw this.deps.notFound(`memory not found: ${id}`);
        ...
        if (memory.memoryLayer === "Skill") {
            return {
                skillTrials: this.deps.repos.runtime.listSkillTrials({
                    ...
                    limit: 100
                }).map((trial) => skillTrialRef(trial, trial.episodeId ? this.deps.repos.runtime.getEpisode(trial.episodeId) : undefined))
            };
        }
        return {};
    }
}
export function episodeRef(episode) {
...
export function episodeSkillStatus(episode) {
    if (episode.skillMemoryIds.length > 0)
        return "succeeded";
    if (episode.pipelineStatus === "running")
        return "running";
    if (episode.pipelineStatus === "failed")
        return "failed";
...
export function episodeSkillReason(episode) {
    if (episode.skillMemoryIds.length > 0)
        return "已从该任务沉淀出可复用技能。";
    if (episode.pipelineError?.trim())
        return `技能沉淀失败：${episode.pipelineError.trim()}`;
    if (typeof episode.rTask === "number" && episode.rTask <= -0.5) {
        return `任务评分 ${episode.rTask.toFixed(2)}，被视为反例；不会沉淀出新的经验或技能。`;
    }
    if (episode.rewardDetail.skipped === true) {
        if (episode.turnCount < 2)
            return "对话轮次不足，需要至少 2 轮完整问答才能生成摘要或技能。";
        return "Reward 评分被跳过，暂不生成技能。";
    }
    if (episode.meta.closeReason === "abandoned")
    ...
    if (episode.pipelineStatus === "running")
        return "正在沉淀技能。";
    if (episode.pipelineStatus === "succeeded")
        return "本任务未产出可复用技能。";
...
export function policyLinkRef(link) {
    return {
...
export function skillTrialRef(trial, episode) {
    return {
        trialId: trial.id,
        status: trial.status,
...
export function timelineMemoryIdsForEpisode(episode, layers) {
    return [
...
export function episodeDetailItem(episode, timeline, sourceMemoryIds, refs) {
...
export function renderEpisodeDetailBody(episode, timeline, title) {
    const lines = [
        `Episode: ${title}`,
        `Status: ${episode.status}`,
    ...
    if (timeline.rawTurns?.length) {
        ...
        timeline.rawTurns.forEach((turn, index) => {
            ...
            if (turn.userText)
                lines.push(`   user: ${truncateEpisodeLine(turn.userText)}`);
            if (turn.assistantText)
                lines.push(`   assistant: ${truncateEpisodeLine(turn.assistantText)}`);
            if (turn.toolCalls?.length)
                lines.push(`   toolCalls: ${turn.toolCalls.length}`);
            if (turn.toolResults?.length)
                lines.push(`   toolResults: ${turn.toolResults.length}`);
        });
    }
...
export function truncateEpisodeLine(value, maxChars = 220) {
...
function truncateDetailTitle(value) {
