...
export class PanelReadModel {
    ...
    panelItems(input) {
        ...
        const scopes = this.deps.repos.l3WorldModels.getScopesByMemoryIds(memories.filter((memory) => memory.memoryLayer === "L3").map((memory) => memory.id));
        const scopesByMemoryId = new Map(scopes.flatMap((scope) => scope.memoryId ? [[scope.memoryId, scope]] : []));
        return {
            items: memories.map((memory) => {
                const item = panelListItemFromMemory(this.deps.repos.memories.toListItem(memory), memory, this.deps.repos.processing.get(memory.id));
...
function panelWorldModelScope(scope) {
    if (!scope.projectId)
        return { kind: "general" };
...
export function workspaceUriDisplay(workspaceUri) {
    if (!workspaceUri)
...
export function changeLogToPanelChange(change) {
    return {
        seq: change.seq,
...
function userMemoryPanelItem(memory) {
