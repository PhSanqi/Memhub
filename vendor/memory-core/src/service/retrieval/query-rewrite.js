export function normalizeQueryRewriteQueries(value, maxQueries) {
...
export function mergeRetrievalResults(retrievals, limit, rrfConstant, minPerQuery) {
    ...
    for (const [queryIndex, retrieval] of retrievals.entries())
        for (const [rank, hit] of retrieval.hits.entries()) {
            ...
            const entry = existing ?? { hit, score: 0, firstRank: rank, firstQueryIndex: queryIndex };
            entry.score += 1 / (rank + 1 + rrfConstant);
...
function recallHitTier(hit) { return hit.memoryLayer === "Skill" ? "tier1" : hit.memoryLayer === "L3" ? "tier3" : "tier2"; }
