...
export function appendJobChange(deps, job, op, before) {
...
export function appendEmbeddingRetryChange(deps, retry, op, before, error) {
...
export function appendEpisodeChange(deps, episode, input) {
...
export function enqueueEmbeddingRetry(deps, memory, sourceText, at, vectorField = embeddingRetryVectorFieldForMemory(memory)) {
...
export function enqueueJob(deps, input) {
...
export async function processJob(deps, job) {
    switch (job.jobType) {
...
export function closeIdleEpisodesForMemoryWrite(deps, job) {
...
export function enqueueEpisodeReflection(deps, episode, at, trigger) {
    if (episode.status !== "closed" ||
        episodeHasPendingCaptureDecision(deps, episode) ||
...
function episodeHasPendingCaptureDecision(deps, episode) {
...
export function enqueueImportSummaryIfMissing(deps, memory, at) {
    const jobType = memoryHasImportPipeline(memory) ? "import_summary" : "trace_summary";
    if (deps.repos.runtime.hasPendingJob(memory.id, jobType, memory.contentHash ?? undefined))
        return;
    deps.repos.transaction(() => {
...
export function episodeHasRewardForReflection(deps, episode) {
    if (episode.status !== "closed" ||
    ...
    const traceIds = Array.isArray(episode.rewardDetail.traceIds)
        ? episode.rewardDetail.traceIds.filter((id) => typeof id === "string")
        : [];
    const activeL1MemoryIds = episode.l1MemoryIds.filter((id) => deps.repos.memories.get(id)?.status === "activated");
    return traceIds.length === activeL1MemoryIds.length &&
        traceIds.every((id, index) => id === activeL1MemoryIds[index]);
}
export function episodeRewardWasSkipped(episode) {
...
export function processingJobMatchesMemory(job, memory) {
...
export function evolutionJobDedupeKey(input) {
    const payload = input.payload ?? {};
    const payloadString = (key) => {
...
export function createWorkerJobHandlers(deps) {
    return {
        processJob: (job) => processJob(deps, job),
        closeIdleEpisodesForMemoryWrite: (job) => closeIdleEpisodesForMemoryWrite(deps, job),
        appendJobChange: (job, op, before) => appendJobChange(deps, job, op, before),
        appendEmbeddingRetryChange: (retry, op, before, error) => appendEmbeddingRetryChange(deps, retry, op, before, error),
        appendEpisodeChange: (episode, input) => appendEpisodeChange(deps, episode, input),
        enqueueEmbeddingRetry: (memory, sourceText, at, vectorField) => enqueueEmbeddingRetry(deps, memory, sourceText, at, vectorField),
        enqueueJob: (input) => enqueueJob(deps, input),
        finalizeClosedEpisode: (episode, at, trigger) => finalizeClosedEpisode(deps, episode, at, trigger),
        enqueueEpisodeRewardAfterReflection: (episode, at, trigger) => enqueueEpisodeRewardAfterReflection(deps, episode, at, trigger),
        enqueueEpisodeReflection: (episode, at, trigger) => enqueueEpisodeReflection(deps, episode, at, trigger),
        enqueueImportSummaryIfMissing: (memory, at) => enqueueImportSummaryIfMissing(deps, memory, at)
    };
}
