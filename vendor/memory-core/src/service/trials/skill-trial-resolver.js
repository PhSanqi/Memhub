import { skillEtaAfterTrial, skillMetaFromMemory, skillStatusAfterTrial, traceMetaFromMemory } from "../../algorithm/plugin-algorithms.js";
...
export class SkillTrialResolver {
    ...
    feedbackForTrial(trial, payload) {
    ...
    pendingTrialsForFeedback(feedback) {
    ...
    updateSkillTrialStats(trial, at) {
        ...
        recordApiLog(this.deps.repos.runtime, "skill_evolve", { phase: "done", skillId: saved.id, trialId: trial.id, reason: "skill_trial_update" }, {
            skillId: saved.id,
    ...
    outcomeForSkillTrial(trial, payload, feedback) {
        ...
        const explicitVerifier = firstBoolean(payload.verifierPassed, payload.fullPass, payload.passed, payload.verifier_passed, payload.full_pass);
        if (explicitVerifier === true)
            return "success";
        if (explicitVerifier === false)
            return "failure";
    ...
    resolveSkillTrialEvidence(request, session, episode) {
...
function outcomeFromFeedback(feedback) {
...
function outcomeFromReward(rHuman, config) {
    if (rHuman >= config.outcomeRTaskSuccessThreshold)
        return "success";
    if (rHuman <= config.outcomeRTaskFailureThreshold)
        return "failure";
    return "unknown";
}
function skillTrialStatusFromOutcome(outcome) {
    if (outcome === "success")
        return "pass";
...
function firstBoolean(...values) {
    for (const value of values) {
        ...
        if (typeof value === "string") {
            ...
            if (["true", "pass", "passed", "success", "succeeded", "ok"].includes(normalized))
                return true;
            if (["false", "fail", "failed", "failure", "error"].includes(normalized))
                return false;
        }
    }
    return undefined;
