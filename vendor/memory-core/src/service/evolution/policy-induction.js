import { L2_INDUCTION_PROMPT, buildPolicyDraft, detectDominantLanguage, l2CandidateIdFor, languageSteeringLine, packL2InductionTraces, policyMetaFromMemory, signatureFromTrace, skillMetaFromMemory, traceMetaFromMemory, tracePolicySimilarity } from "../../algorithm/plugin-algorithms.js";
...
export class PolicyInductionEngine {
    ...
    async induceL2(job) {
        ...
        for (const signature of signatures) {
            ...
            const distinctEpisodeCount = uniq(bucket
            ...
            if (matchingPolicy) {
                ...
                for (const episodeId of uniq(bucket.map((trace) => trace.episodeId).filter((id) => Boolean(id)))) {
    ...
    associateL2(job) {
        ...
        if (!trace || !this.isTraceEligibleForL2(trace))
            return;
    ...
    async enhancePolicyDraft(signature, evidenceTraces, fallback) {
        ...
        try {
            ...
            for (let attempt = 0; attempt < L2_INDUCTION_DRAFT_MAX_ATTEMPTS; attempt += 1) {
                const result = await this.deps.skillLlm.completeJson([
                    ...
                    {
                        role: "user",
                        content: packL2InductionTraces(evidenceTraces, this.deps.config.algorithm.l2Induction.traceCharCap, signature)
                    }
                ], {
    ...
    findExistingPolicyForL2Bucket(signature, evidenceTraces) {
    ...
    recomputePolicyStats(policyId, at, triggerEpisodeId) {
        ...
        if (!policy)
            return undefined;
        const linkedTraceIds = new Set(this.deps.repos.runtime
        ...
        const nextStatus = policy.status === "active" && stats.status !== "active"
    ...
    l2GainReferenceTraces(evidenceTraces, triggerEpisodeId) {
        const episodeIds = new Set(evidenceTraces
            .map((trace) => trace.episodeId)
        ...
        for (const trace of this.deps.repos.memories
            ...
            .map((memory) => this.deps.traceMeta(memory))
            .filter((trace) => Boolean(trace && trace.episodeId && episodeIds.has(trace.episodeId) && Number.isFinite(trace.value)))) {
            byId.set(trace.id, trace);
        }
        return [...byId.values()].sort((a, b) => a.ts - b.ts || a.id.localeCompare(b.id));
    }
    recordCandidatePoolTrace(trace, signature, at) {
    ...
    markCandidatePoolPromoted(userId, signature, sourceMemoryIds, policyId, at) {
...
export function updatePolicyStats(memory, input) {
...
function renderPolicyBody(draft) {
    return [
        draft.title,
        `Trigger: ${draft.trigger}`,
        `Procedure: ${draft.procedure}`,
...
function l2InductionInvalidReason(result) {
    if (!isRecord(result))
        return "llm-failed: l2.induction.invalid: non-object output";
    if (!firstString(result.title))
        return "llm-failed: l2.induction.invalid: missing title";
    if (!firstString(result.trigger))
        return "llm-failed: l2.induction.invalid: missing trigger";
    if (!firstString(result.procedure, result.action)) {
        return "llm-failed: l2.induction.invalid: missing procedure";
    }
    if (!firstString(result.boundary) &&
...
function inferPolicyFreshness(text) {
