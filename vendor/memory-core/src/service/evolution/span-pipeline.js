import { BATCH_REFLECTION_PROMPT, REFLECTION_SCORE_PROMPT, detectDominantLanguage, languageSteeringLine, traceMetaFromMemory } from "../../algorithm/plugin-algorithms.js";
...
import { importStatusTags, memoryHasImportPipeline, updateImportPipelineStatus } from "../import/import-job-processor.js";
...
export class SpanPipeline {
    ...
    async reflectSingleTrace(job, memory, trace) {
    ...
    async reflectEpisodeBatch(job) {
        ...
        const primary = await this.runBatchReflectionWindowPass(episode, memories, cfg.reflectionBatchWindowSize, cfg.reflectionBatchOverlap, cfg.reflectionBatchPrimaryMaxRetries);
        if (primary.success) {
            await this.applyBatchReflectionScores(job, memories, mergeBatchWindowScores(memories.length, primary.results));
            return true;
        }
        const degraded = await this.runBatchReflectionWindowPass(episode, memories, cfg.reflectionBatchDegradedWindowSize, cfg.reflectionBatchDegradedOverlap, cfg.reflectionBatchDegradedMaxRetries);
        if (degraded.success) {
        ...
        await this.applyBatchReflectionScores(job, memories, batchRelatedDefaultScores(memories.length, Array.isArray(payload.steps) ? payload.steps : undefined));
        return true;
    }
    async runBatchReflectionWindowPass(episode, memories, windowSize, overlap, maxRetries) {
        ...
        for (const win of windows) {
            ...
            if (!ok && failedWindows === 0)
                failedWindows += 1;
        }
        return { success: failedWindows === 0, results, failedWindows };
    }
    async scoreBatchReflectionWindow(episode, memories) {
        const payload = this.batchReflectionPayload(episode, memories);
        ...
        return Array.from({ length: steps.length }, (_, idx) => {
            ...
            if (!score)
                throw new Error(`batch reflection score missing idx: ${idx}`);
            return score;
        });
    }
    async applyBatchReflectionScores(job, memories, scores) {
        const at = nowIso();
    ...
    applyUnconfiguredEpisodeDefault(job) {
        if (!job.episodeId)
            return false;
        const episode = this.deps.repos.runtime.getEpisode(job.episodeId);
        if (!episode || episode.status !== "closed" || episode.l1MemoryIds.length === 0)
        ...
        for (const candidate of memories) {
            if (traceReflectionWasScored(candidate))
                continue;
    ...
    applyUnconfiguredTraceDefault(job, memory, trace, at = nowIso()) {
        const next = updateImportPipelineStatus(updateTraceReflection(memory, {
    ...
    batchReflectionPayload(episode, memories) {
        const cfg = this.deps.config.algorithm.capture;
    ...
    reflectionTaskSummary(job, fallback) {
    ...
    reflectionDownstreamPreview(job, memory) {
        const cfg = this.deps.config.algorithm.capture;
        if (!job.episodeId ||
    ...
    async summarizeTraceForCapture(input, options = {}) {
        const messages = [
            {
                role: "system",
...
function updateTraceReflection(memory, input) {
    const trace = traceMetaFromMemory(memory);
    if (!trace) {
        return memory;
    }
    const internalTrace = isRecord(memory.properties.internal_info.trace)
        ...
        : {};
    const { summary_deferred_until_reflection: _summaryDeferredUntilReflection, ...settledTrace } = internalTrace;
    const nextTrace = {
        ...settledTrace,
...
function isDurableMemoryBatchReflectionStep(step) {
    if (!step)
        return false;
    const text = [
...
function parseBatchReflectionRelevance(value) {
    if (value === "IRRELEVANT" || value === "RELATED" || value === "PIVOTAL")
        return value;
    throw new Error(`batch reflection relevance invalid: ${String(value)}`);
}
function alphaForBatchReflectionRelevance(value) {
    if (value === "IRRELEVANT")
        return 0;
    if (value === "PIVOTAL")
        return 1;
...
function isSocialOnlyBatchReflectionStep(step) {
    if (!step)
        return false;
    ...
    if (!state)
        return false;
    if (batchReflectionWordCount(state) > 6)
        return false;
    if (isDurableMemoryBatchReflectionStep(step))
        return false;
    const explicitSocialPattern = /(谢谢|感谢|辛苦|客气|不用谢|再见|拜拜|你好|您好|早上好|晚上好)|\b(?:thanks?|thank\s+you|appreciate|great\s+job|well\s+done|awesome|nice|you(?:'|’)re\s+welcome|no\s+problem|bye|goodbye|hello|hi)\b/i;
    const shortPraisePattern = /^(?:你|您|回答|做得)?[^\n]{0,8}(?:真棒|棒极了|做得好|很好|很对|厉害|太强了)[！!。.]*$/i;
    const substantiveSignalPattern = /[?？]|(?:请|帮我|推荐|介绍|解释|分析|比较|查询|查找|查一下|告诉我|为什么|怎么|如何|什么|哪(?:个|种|里)?|是否|是不是|能否|需要|想要|问题|关于|区别|原因)|\b(?:please|help|recommend|introduce|explain|analy[sz]e|compare|find|search|tell\s+me|why|how|what|which|where|whether|can\s+you|could\s+you|need|want|question|about)\b/i;
    const taskSignalPattern = /(修复|实现|改|更新|测试|报错|错误|命令|脚本|代码|函数|文件|数据库|sql|trace|episode|reward|reflection|alpha|value|fix|implement|update|test|error|command|script|code|function|file|db|database|query|bug|issue|task)/i;
...
function batchReflectionWordCount(text) {
    return Array.from(new Intl.Segmenter(undefined, { granularity: "word" }).segment(text))
        .filter((part) => part.isWordLike)
        .length;
}
function socialOnlyBatchReflectionScore(idx) {
    return {
        idx,
...
function mergeBatchWindowScores(length, windowScores) {
...
function batchRelatedDefaultScores(length, steps) {
...
function batchReflectionRank(score) {
...
function buildBatchWindows(length, windowSize, overlap) {
    if (length <= 0)
        return [];
    const out = [];
...
function batchTaskContext(episode, rawTurns, maxChars = 1200) {
    const parts = [
...
export function traceSortKey(memory) {
    const trace = traceMetaFromMemory(memory);
    if (!trace)
        return Date.parse(memory.timeline);
    return Number.isFinite(trace.ts) ? trace.ts : Date.parse(memory.timeline);
}
function traceReflectionSource(memory) {
...
export function traceReflectionWasScored(memory) {
    const internalTrace = isRecord(memory.properties.internal_info.trace)
        ? memory.properties.internal_info.trace
        : {};
    return typeof internalTrace.reflection_scored_at === "string";
}
function sanitizeReflectionText(value) {
    return value
...
function reflectionContextIncludesDownstream(mode) {
    return mode === "downstream" || mode === "task_downstream";
}
function reflectionContextIncludesTask(mode) {
    return mode === "task" || mode === "task_downstream";
}
function traceDownstreamPreviewBlock(memory, offset, maxChars) {
    const trace = traceMetaFromMemory(memory);
    if (!trace) {
        return "";
    }
...
function traceSummaryPayload(input, includeToolOutput = false) {
    const parts = [`CAPTURED AT: ${formatZonedTime(input.trace.ts, input.trace.timeZone)}`];
    if (input.userText) {
    ...
    if (input.toolCalls.length > 0) {
        parts.push(`TOOLS:\n${clip(input.toolCalls.map((call) => includeToolOutput
            ? `${call.name}(${clip(stringifyForMemory(call.input), 180)}) -> ${clip(stringifyForMemory({ output: call.output, error: call.error }), 500)}`
            : `${call.name}(${clip(stringifyForMemory(call.input), 120)})`).join("; "), includeToolOutput ? 1_600 : 400)}`);
    }
...
function formatReflectionToolCall(call) {
...
function lastReflectionToolOutcome(toolCalls, maxChars) {
...
function traceAgentThinking(memory) {
    const trace = memory.properties.internal_info.trace;
    if (!trace || typeof trace !== "object" || Array.isArray(trace)) {
        return undefined;
    }
    const thinking = trace.agent_thinking;
