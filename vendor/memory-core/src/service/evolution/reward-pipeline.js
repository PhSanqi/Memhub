...
import { SPAN_BIG_TURN_ENABLED, SPAN_BIG_TURN_MIN_TOOL_CALLS } from "./big-turn-span-pipeline.js";
...
export class RewardPipeline {
    ...
    async applyReward(job) {
        ...
        const payloadHasFeedback = typeof job.payload.polarity === "string" ||
        ...
        const episodeTraces = this.deps.repos.memories
            ...
            .filter((item) => Boolean(item && item.episodeId === trace.episodeId))
            .sort((a, b) => a.ts - b.ts);
    ...
    async scoreFeedbackWithLlm(input) {
        if (!this.deps.config.algorithm.reward.llmScoring || !this.deps.llm.isConfigured())
        ...
        try {
            ...
            if (typeof rawGoalAchievement !== "number" || !Number.isFinite(rawGoalAchievement) ||
    ...
    rewardSourceForJob(job) {
        ...
        return episodeMemories
            ...
            .filter((item) => Boolean(item.trace && item.trace.episodeId === job.episodeId))
            .sort((a, b) => a.trace.ts - b.trace.ts)[0];
    }
    async maybeCreateValueDistributionRepair(triggerMemory, at) {
        const triggerTrace = this.deps.traceMeta(triggerMemory);
        if (!triggerTrace?.signature)
            return undefined;
        const evidence = this.valueDistributionRepairEvidence(triggerTrace, this.deps.config.algorithm.feedback.evidenceLimit);
        if (evidence.highValueMemories.length === 0 || evidence.lowValueMemories.length === 0)
            return undefined;
        ...
        if (cooldownMs > 0) {
            ...
            if (recent.length > 0)
                return { contextHash, skipped: true, reason: "cooldown" };
        }
        ...
        if (triggerTrace.episodeId)
            this.deps.repos.runtime.appendEpisodeDecisionRepair(triggerTrace.episodeId, repair.id, at);
        this.deps.repos.runtime.appendChange({
            memoryId: repair.id,
    ...
    valueDistributionRepairEvidence(trace, limit) {
        ...
        for (const memory of memories) {
            ...
            if (!candidate || candidate.signature !== trace.signature)
                continue;
            if (candidate.value > 0 && highValueMemories.length < limit)
                highValueMemories.push(memory);
            if (candidate.value < -this.deps.config.algorithm.feedback.minLowValueThreshold && lowValueMemories.length < limit)
    ...
    async maybeSynthesizeValueDistributionDecisionRepair(contextHash, trace, evidence) {
        const request = {
            trigger: "value-distribution",
            contextHash,
...
export function buildRewardEpisodeInput(input) {
    const traces = input.episodeTraces.length
        ...
        : [input.trace];
    const perTurnCap = Math.min(200, Math.max(1, Math.floor(Math.max(input.summaryMaxChars, traces.length) / traces.length)));
...
function rewardEpisodeMission(episode, fallback) {
...
function rewardTurnSummary(trace, maxChars) {
    const summary = trace.summary.trim();
    if (summary)
        return rewardOneLine(summary, maxChars);
    const userText = trace.userText.trim();
    const agentText = trace.agentText.trim();
    if (!userText || !agentText)
        return rewardOneLine(userText || agentText || "trace memory", maxChars);
    const separator = " → ";
    if (maxChars <= separator.length + 2)
        return rewardOneLine(`${userText}${separator}${agentText}`, maxChars);
...
function rewardExecutionOutcome(traces) {
...
function rewardFeedbackInput(payload) {
...
function rewardHostInput(source, episode) {
    const meta = episode?.meta ?? {};
    const hints = isRecord(meta.contextHints) ? meta.contextHints : {};
    const field = (value) => typeof value === "string" && value.trim() ? rewardOneLine(value, 120) : undefined;
    ...
    if (!agent && !agentIdentity && !provider && !model && !apiMode)
        return undefined;
    return {
        ...(agent ? { agent } : {}),
...
export function rewardSkipReason(traces, cfg) {
    let userTurns = 0;
...
function rewardLooksLikeTrivialContent(text) {
...
export function updateTraceScore(memory, input) {
    ...
    const sourceFeedbackIds = uniq([
        ...
        ...(trace && typeof trace === "object" ? stringArray(trace.source_feedback_ids) : []),
        ...(input.sourceFeedbackId ? [input.sourceFeedbackId] : [])
    ]);
    const nextTrace = trace && typeof trace === "object"
        ? { ...trace, value: input.value, alpha: input.alpha, priority: input.priority, r_human: input.rHuman, reward_reason: input.rewardReason, ...(sourceFeedbackIds.length > 0 ? { source_feedback_ids: sourceFeedbackIds } : {}) }
...
export function valueDistributionPreference(memory) {
...
export function valueDistributionAntiPattern(memory) {
    const trace = memory ? traceMetaFromMemory(memory) : null;
    const text = trace?.reflection ?? trace?.agentText ?? trace?.summary;
...
function rewardOneLine(value, maxChars) {
