...
export class BigTurnSpanPipeline {
    ...
    async splitAndStore(job) {
        ...
        const rawTurn = rawTurnId
            ...
            : undefined;
        if (!source || !rawTurn || rawTurn.toolCalls.length < SPAN_BIG_TURN_MIN_TOOL_CALLS)
            return;
    ...
    linkSpansToSourceTrace(sourceTraceId, spanIds, job) {
...
function spanId(sourceTraceId, span) {
    return `span_${stableHash(`${sourceTraceId}:${span.start}:${span.end}`).slice(0, 20)}`;
}
function bigTurnPromptPayload(source, rawTurn, job) {
    ...
    return {
        ...
        traceSummary: redactAndClip(text(trace.summary) ?? text(source.info.summary) ?? "", 1_000),
        reflection: redactAndClip(text(trace.reflection) ?? text(internal.reflection) ?? "", 1_000),
        reward: {
...
function redactAndClip(value, maxChars) {
    return clip(redactSensitiveText(value), maxChars);
}
function validateSpanResult(result, toolCallCount) {
    ...
    const spans = result.spans.map((value) => {
        if (!isRecord(value))
            throw new Error("span.big_turn returned invalid span");
