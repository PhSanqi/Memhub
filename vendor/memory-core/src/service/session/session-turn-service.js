import { captureTurnSteps, classifyIntent, classifyTurnFeedback, classifyTurnRelation, classifyTurnRelationWithLlm, policyMetaFromMemory, retrievalLayersForMode, retrievePluginMemories, signatureFromTraceParts, traceMetaFromMemory } from "../../algorithm/plugin-algorithms.js";
import {} from "../../config/index.js";
import { jobToRef, L3WorldModelScopeWorkspaceConflictError, Repositories } from "../../storage/repositories.js";
...
import { buildUserMemory, classifyUserMemory, isDynamicCurrentFactQuery, isPureUserMemoryStatement, isQuestionLike, isTaskLinkedUserFeedback, isUserMemoryQuestion } from "../user-memory/user-memory.js";
import { recordApiLog } from "../model-audit/model-call-audit.js";
import { namespaceForRawTurn, namespaceForSession, normalizeNamespace, resolveV2WorkspaceIdentityForOpenRequest, sessionScopeForOpenRequest } from "../namespace/namespace-scope.js";
import { detailSummaryForMemory, detailTitleForMemory, firstDetailDisplayString } from "../read-model/memory.js";
import { buildRepairSuggestionQuery, buildSearchQuery, completeObservedRawTurn, normalizeCompleteTurnArtifacts, normalizeCompleteTurnSourceMemoryIds, normalizeCompleteTurnToolCalls, normalizeCompleteTurnToolResults, rawTurnIdForSessionTurn, sanitizeTurnCompleteRequest, sanitizeTurnStartRequest, turnStartContextHints } from "../turn/turn-normalization.js";
export function toolObservationEvent(input) {
    ...
    const event = { id: input.toolCallId, name: input.toolName, input: input.args, output: input.error === undefined ? input.result : undefined, error, success };
...
export function toolOutcomeFromObservation(input, rawTurn, updatedRawTurn) {
...
export function toolRepairContext(session, episode) { return [session.userId, session.projectId ?? session.workspaceId ?? session.conversationId ?? "default", episode.id].join(":"); }
export function toolSignalKey(toolId, context) { return `${toolId}|${context}`; }
export function toolRepairContextHash(toolId, context) { return stableHash(`${toolId}\n${context}`).slice(0, 16); }
export function repairEvidenceValueDiff(high, low) { if (!high.length || !low.length)
    return Number.POSITIVE_INFINITY; return Math.abs(meanTraceValue(high) - meanTraceValue(low)); }
export function isRepairFailureLikeTrace(trace) { const blob = `${trace.agentText}\n${trace.reflection ?? ""}`.toLowerCase(); return /(error|failed|failure|exception|traceback|timeout|retry)/.test(blob) || trace.toolCalls.some((call) => Boolean(call.error ?? errorMessageFromUnknown(call.output))); }
export function repairTraceContains(trace, needle) { return `${trace.userText}\n${trace.agentText}\n${trace.reflection ?? ""}`.toLowerCase().includes(needle); }
function matchingObservedToolCall(record, rawTurn, updatedRawTurn) { const id = stringFromMaybeRecord(record, "id") ?? stringFromMaybeRecord(record, "toolCallId"); const name = stringFromMaybeRecord(record, "name") ?? stringFromMaybeRecord(record, "toolName"); const calls = [...updatedRawTurn.toolCalls, ...rawTurn.toolCalls].filter(isToolCallPayload); return calls.find((call) => id && call.id === id) ?? calls.find((call) => name && call.name === name); }
function successFromToolObservation(record) { if (!record)
    return undefined; if (typeof record.success === "boolean")
    return record.success; if (typeof record.ok === "boolean")
    return record.ok; if (typeof record.exitCode === "number")
    return record.exitCode === 0; if (typeof record.status === "string") {
    const status = record.status.toLowerCase();
    if (["succeeded", "success", "ok", "passed"].includes(status))
        return true;
    if (["failed", "failure", "error", "cancelled"].includes(status))
        return false;
} return record.error !== undefined ? false : undefined; }
function failureReasonFromToolObservation(event, call) { const reason = errorMessageFromUnknown(event) ?? (isRecord(event) ? stringFromMaybeRecord(event, "output") : undefined) ?? call?.error ?? errorMessageFromUnknown(call?.output); return reason ? clip(reason, 240) : undefined; }
function meanTraceValue(memories) { const values = memories.map((memory) => traceMetaFromMemory(memory)?.value).filter((value) => typeof value === "number" && Number.isFinite(value)); return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : 0; }
function errorMessageFromUnknown(value) { if (value === undefined || value === null)
    return undefined; if (value instanceof Error)
    return value.message; if (typeof value === "string")
    return value; if (isRecord(value)) {
    const message = value.error ?? value.message;
    if (typeof message === "string")
        return message;
} return undefined; }
function stringFromMaybeRecord(record, key) { return isRecord(record) ? (typeof record[key] === "string" ? record[key] : undefined) : undefined; }
function isToolCallPayload(value) { return isRecord(value) && typeof value.name === "string"; }
function uniq(values) { return [...new Set(values)]; }
function objectField(value, key) { return isRecord(value) && typeof value[key] === "string" ? value[key] : undefined; }
const EXPLICIT_END_TOPIC_COMMANDS = new Set([
    "结束会话",
    "不聊了"
]);
function turnRouteProposalFromRecallRequest(request) {
    if (!isRecord(request) || !isRecord(request.routeProposal))
        return undefined;
    ...
    if ((action !== "create_first" && action !== "append" && action !== "split" && action !== "end_topic") ||
        !decision ||
...
function turnIntentDecisionFromRecallRequest(request) {
    if (!isRecord(request) || !isRecord(request.turnIntentDecision))
        return undefined;
...
function turnContextPacketId(sessionId, baseEpisodeId, turnId, searchEventId) {
    return `ctx_${stableHash(`${sessionId}:${baseEpisodeId ?? "unbound"}:${turnId}:${searchEventId}`).slice(0, 20)}`;
}
function explicitEndTopicDecision(text) {
    const command = text
        .trim()
        .toLowerCase()
        .replace(/[。！？!?.,，、;；:："'“”‘’]+$/gu, "")
...
function endTopicDecisionFromRawTurn(rawTurn) {
    ...
    if (close?.closeAfterComplete !== true ||
        decision?.relation !== "end_topic" ||
        typeof decision.confidence !== "number" ||
        typeof decision.reason !== "string" ||
        !Array.isArray(decision.signals) ||
...
function rawTurnIsExcludedFromL1(rawTurn) {
    ...
    if (kind === "chitchat" || kind === "meta" || kind === "memory_probe")
        return true;
    if (isUserMemoryQuestion(rawTurn.userText ?? ""))
        return true;
    if (isDynamicCurrentFactQuery(rawTurn.userText ?? ""))
        return true;
    const taskLinkedFeedback = isTaskLinkedUserFeedback(rawTurn.userText ?? "");
    const hasOnlyRecalledMemoryEvidence = rawTurn.sourceMemoryIds.length > 0 && ((rawTurn.toolCalls.length === 0 && rawTurn.toolResults.length === 0) ||
        (rawTurn.toolCalls.length > 0 && rawTurn.toolCalls.every((call) => isToolCallPayload(call) && isMemmyRecallToolName(call.name))));
    if (!taskLinkedFeedback &&
        hasOnlyRecalledMemoryEvidence)
        return true;
    if (isPureUserMemoryStatement(rawTurn.userText ?? "") &&
...
function l1ObservationMetadata(rawTurn, session, at) {
...
function pendingL1DecisionMetadata(observation) {
...
export function rawTurnSummary(rawTurn) {
...
export class SessionTurnService {
    deps;
    toolFailureStates = new Map();
    toolSuccessSteps = new Map();
    toolStepCounters = new Map();
    constructor(deps) {
        this.deps = deps;
    }
    openSession(request) {
        ...
        if (idempotencyKey) {
            ...
            if (existing) {
                ...
                return this.deps.withDuplicateFlag(existing.response);
            }
        }
        const namespace = normalizeNamespace(request.namespace);
        const at = nowIso();
        if (request.l3WorldModelProtocolVersion === undefined && (request.l3WorldModelTransition !== undefined ||
        ...
        if (request.sessionId) {
            ...
            if (existingSession) {
                ...
                const refreshed = this.deps.repos.runtime.updateSessionScope(existingSession.id, sessionScopeForOpenRequest(request, namespace), at) ?? existingSession;
                const body = {
        ...
        if (hostSessionKey) {
            ...
            if (existingSession) {
                this.deps.assertSessionInScope(existingSession, request.namespace);
                const touched = this.deps.repos.runtime.updateSessionScope(existingSession.id, sessionScopeForOpenRequest(request, namespace), at) ?? existingSession;
                const body = {
    ...
    openV2Session(request, namespace, at) {
        ...
        try {
            ...
            throw new MemoryServiceError("invalid_argument", error instanceof Error ? error.message : "invalid workspace identity");
        }
        return this.deps.repos.transaction(() => {
            ...
            if (explicitProjectId || explicitWorkspaceId) {
                throw new MemoryServiceError("invalid_argument", "protocol v2 derives projectId and workspaceId from workspace identity");
            }
            const session = {
                id: newId("session"),
                userId: namespace.userId,
    ...
    bindV2SessionWorkspace(session, workspaceUri, at) {
    ...
    assertV2SessionIdentity(session, request, namespace, workspace) {
    ...
    closeLegacySessionForV2Rollover(session, at) {
    ...
    v2SessionOpenBody(session, resumed) {
        return {
            sessionId: session.id,
            userId: session.userId,
            source: session.source,
    ...
    async startTurn(request) {
        ...
        const routeProposalPromise = this.proposeEpisodeRouteWithLlm(latestEpisode, request.query, endTopicDecision);
        ...
        const requestedLayers = request.layers === undefined
            ? intentLayers
            : intentLayers.filter((layer) => request.layers?.includes(layer));
        ...
        const contextPacketId = turnContextPacketId(session.id, routeProposal.baseEpisodeId, turnId, search.searchEventId);
    ...
    completeTurn(turnId, request) {
        ...
        if (!request.query.trim() || !request.answer.trim()) {
            throw new MemoryServiceError("invalid_argument", "turn.complete requires a non-empty user query and assistant result");
        }
        if (!this.deps.memoryAddEnabled()) {
        ...
        const response = this.deps.repos.transaction(() => {
            ...
            if (existingRawTurn && isRecord(existingRawTurn.messagePayload?.turn_complete)) {
                ...
                const existingCaptureClaim = existingRawTurn.userText && existingRawTurn.assistantText
                    ? this.deps.repos.captureClaims.get(session.userId, normalizeMemoryCaptureSource(session.source), memoryCaptureQaHash(existingRawTurn.userText, existingRawTurn.assistantText))
                    : undefined;
                const l1MemoryIds = episode.l1MemoryIds.filter((memoryId) => {
            ...
            if (request.episodeId) {
                ...
                const routedEndTopicDecision = endTopicDecision ?? (decision.relation === "end_topic" ? decision : undefined);
                route = {
                    episode,
                ...
                const proposalUsesObservedUnboundEpisode = Boolean((recalledProposal?.action === "create_first" || recalledProposal?.action === "end_topic") &&
                ...
                if (!recalledProposal && existingRawTurn) {
                    ...
                    route = this.commitTurnRouteProposal(session, proposal, request.query, "turn.complete", at, !proposalIsCurrent);
                }
            }
            ...
            if (existingRawTurn && existingRawTurn.episodeId !== episode.id) {
                this.deps.repos.runtime.rebindRawTurnEpisode(existingRawTurn.id, existingRawTurn.episodeId, episode.id, at);
            }
            ...
            const turnStartPayload = {
                ...
                ...(turnStartRecall
                    ? {
                        contextPacketId: turnContextPacketId(session.id, turnStartRecall.episodeId, turnId, turnStartRecall.id),
            ...
            const insertedRawTurn = existingRawTurn ??
            ...
            const l1MemoryIds = [];
            const captureClaimByRawTurnId = new Map();
            let changeSeq = 0;
            const jobs = [...route.jobs, ...userMemoryCapture.jobs];
            for (const step of capturedSteps) {
                ...
                const contentChanged = Boolean(!upsert.created && upsert.previous?.contentHash !== upsert.memory.contentHash);
    ...
    async recordToolOutcomeForRepair(input, session, episode, rawTurn, updatedRawTurn, at) {
        const outcome = toolOutcomeFromObservation(input, rawTurn, updatedRawTurn);
        if (!outcome || outcome.success === undefined)
            return undefined;
    ...
    recordToolSuccess(toolId, context, step) {
        const key = toolSignalKey(toolId, context);
    ...
    async maybeCreateFailureBurstRepair(input) {
    ...
    failureBurstRepairEvidence(input) {
        const query = `${input.toolId}\n${input.reason}`;
        const policies = this.deps.repos.memories.search(query, {
            memoryLayer: "L2",
            status: "activated"
        }, input.limit);
        const policyIds = policies.map((policy) => policy.id);
        const l1Hits = this.deps.repos.memories.search(query, {
            memoryLayer: "L1",
            status: "activated"
        }, input.limit * 4);
        const highValueMemories = [];
        const lowValueMemories = [];
        for (const hit of l1Hits) {
            ...
            if (trace.value < -this.deps.config.algorithm.feedback.minLowValueThreshold &&
    ...
    async maybeSynthesizeFailureBurstDecisionRepair(burst, reason, evidence) {
    ...
    async repairSuggestion(input) {
        ...
        }, 500).filter((memory) => this.deps.isMemoryReadyForRetrieval(memory));
        ...
        const retrievedRawTurnIds = new Set(retrievedMemories
    ...
    captureEpisodeIncrementalSteps(episode, currentRawTurn, at) {
        const seenRawTurnIds = new Set(episode.l1MemoryIds
        ...
        const rawTurns = uniq([...episode.rawTurnIds, currentRawTurn.id])
            ...
            .filter((rawTurn) => Boolean(rawTurn && (rawTurn.id === currentRawTurn.id || !seenRawTurnIds.has(rawTurn.id))))
        ...
        return rawTurns.flatMap((rawTurn) => captureTurnSteps({
            episodeId: episode.id,
    ...
    captureUserMemory(rawTurn, request, at) {
        ...
        if (correction) {
            ...
            this.appendUserMemoryChange(inserted, upsert.previous, upsert.created ? "created" : "updated", at);
            if (archived)
                this.appendUserMemoryChange(archived, target, "archived", at);
            return {
        ...
        this.appendUserMemoryChange(upsert.memory, upsert.previous, upsert.created ? "created" : "updated", at);
        return {
            memoryIds: [upsert.memory.id],
            jobs: upsert.created ? this.userMemoryEmbeddingJobs(upsert.memory, rawTurn, at) : []
        };
    }
    userMemoryEmbeddingJobs(memory, rawTurn, at) {
    ...
    appendUserMemoryChange(memory, before, op, at) {
        this.deps.repos.runtime.appendChange({
            memoryId: memory.id,
            kind: "user_memory",
            op,
    ...
    buildTurnRouteProposal(latest, decision, lastTurnAtMs, proposedAt = nowIso()) {
        ...
        const gapMs = lastTurnAtMs
            ...
            : 0;
        const withinMergeWindow = this.deps.config.algorithm.session.mergeMaxGapMs === 0 ||
            gapMs <= this.deps.config.algorithm.session.mergeMaxGapMs;
        const shouldAppendOpen = mergeMode &&
            withinMergeWindow &&
            (decision.relation === "revision" ||
                decision.relation === "follow_up" ||
                decision.relation === "unknown");
        const shouldReopenClosed = latest !== undefined && latest.meta.closeReason !== "end_topic" && (decision.relation === "revision" ||
            (mergeMode &&
                withinMergeWindow &&
                (decision.relation === "follow_up" || decision.relation === "unknown")));
        const action = decision.relation === "end_topic"
            ? "end_topic"
            : !latest
                ? "create_first"
                : latest.status === "open"
    ...
    proposeEpisodeRoute(session, userText, forcedDecision) {
        const latest = this.deps.repos.runtime.latestEpisodeForSession(session.id);
    ...
    async proposeEpisodeRouteWithLlm(latest, userText, forcedDecision) {
        const relationContext = latest ? this.episodeRelationContext(latest) : undefined;
    ...
    persistTurnStartRouteProposal(searchEventId, routeProposal) {
    ...
    commitTurnRouteProposal(session, proposal, userText, source, at, proposalStale) {
        const decision = proposal.relationDecision;
        const closedEpisodeIds = [];
        const jobs = [];
    ...
    episodeRelationContext(episode) {
        ...
        const tags = uniq(episode.l1MemoryIds.flatMap((id) => this.deps.repos.memories.get(id)?.tags ?? []));
    ...
    recordImplicitTurnFeedback(session, episode, userText) {
        ...
        if (polarity === "neutral" && classification.magnitude <= 0)
            return;
    ...
    recordRevisionFeedback(session, episode, userText, source) {
        const target = this.deps.feedbackTargetFromEpisode(episode);
        if (!target)
            return;
        const contextHash = stableHash({
...
function v2SessionMeta(request, workspace, timeZone) {
    return {
        ...sanitizedSessionMeta(request.meta),
...
function optionalMetaString(meta, key) {
