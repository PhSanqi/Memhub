...
function parseConfigFile(facts, relativePath, sha256, text) {
    ...
    try {
        if (lower === "package.json") {
            ...
            if (engines) {
                for (const [runtime, version] of Object.entries(engines))
                    add(facts.runtimeDeclarations, `${runtime} ${stringValue(version) ?? ""}`);
            }
            ...
            if (scripts) {
                for (const name of Object.keys(scripts).sort(compare)) {
                    ...
                    if (/^(build|compile|bundle)(:|$)/i.test(name))
                        add(facts.buildEntries, command);
                    if (/^(test|spec)(:|$)/i.test(name))
                        add(facts.testEntries, command);
                    if (/^(lint|check|typecheck|format)(:|$)/i.test(name))
                        add(facts.checkEntries, command);
                }
            }
            return;
        }
        if (/^(tsconfig|jsconfig).*\.json$/i.test(basename) || lower.endsWith(".json")) {
            ...
            if (value && lower.startsWith("tsconfig"))
                add(facts.toolchains, "TypeScript");
        ...
        if (["pyproject.toml", "poetry.lock", "uv.lock", "cargo.toml", "cargo.lock", "rust-toolchain.toml"].includes(lower)) {
            ...
            if (lower === "pyproject.toml") {
                ...
                if (tool?.poetry)
                    add(facts.toolchains, "Poetry");
                if (tool?.uv)
                    add(facts.toolchains, "uv");
                if (tool?.pytest) {
        ...
        if (lower === ".tool-versions") {
            for (const line of text.split(/\r?\n/u))
                add(facts.runtimeDeclarations, line.replace(/\s+/gu, " ").trim());
            return;
        }
        if (lower === "makefile") {
            ...
            for (const target of targets) {
                if (/^(build|all|compile)$/i.test(target))
                    add(facts.buildEntries, `make ${target}`);
                if (/^test/i.test(target))
                    add(facts.testEntries, `make ${target}`);
                if (/^(lint|check|format)/i.test(target))
                    add(facts.checkEntries, `make ${target}`);
            }
            return;
        }
        ...
        if (/^(eslint|jest|vitest)\.config\.(js|cjs|mjs|ts)$/i.test(basename) || /^\.eslintrc\.(js|cjs)$/i.test(basename)) {
            ...
            const hasStaticObject = source.statements.some((statement) => {
                if (ts.isExportAssignment(statement))
                    return staticLiteral(statement.expression) !== undefined;
...
function collectNamedStrings(value, key) {
    ...
    for (const [name, child] of Object.entries(object)) {
        if (name === key && typeof child === "string" && child.trim())
            result.push(child.trim());
        result.push(...collectNamedStrings(child, key));
    }
    return result;
}
function categorizeCommand(facts, add, command) {
    const normalized = command.replace(/\s+/gu, " ").trim();
    if (/\b(build|compile|bundle|package)\b/iu.test(normalized))
        add(facts.buildEntries, normalized);
    if (/\b(test|pytest|vitest|jest)\b/iu.test(normalized))
        add(facts.testEntries, normalized);
    if (/\b(lint|check|typecheck|format|ruff|mypy)\b/iu.test(normalized))
        add(facts.checkEntries, normalized);
}
...
function inferToolchainsFromInventory(facts, entries) {
    const add = (collection, value, path, sha256 = "inventory") => {
    ...
    for (const entry of entries) {
        ...
        if (lower === "package-lock.json")
            add(facts.toolchains, "npm", path, sha);
        else if (lower === "pnpm-lock.yaml")
            add(facts.toolchains, "pnpm", path, sha);
        else if (lower === "yarn.lock")
            add(facts.toolchains, "Yarn", path, sha);
        else if (lower === "bun.lock")
            add(facts.toolchains, "Bun", path, sha);
        else if (lower === "poetry.lock")
            add(facts.toolchains, "Poetry", path, sha);
        else if (lower === "uv.lock")
            add(facts.toolchains, "uv", path, sha);
        else if (lower === "cargo.lock")
            add(facts.toolchains, "Cargo", path, sha);
        else if (lower === "dockerfile" || lower.startsWith("dockerfile."))
            add(facts.toolchains, "Docker", path, sha);
        else if (lower.startsWith(".github/workflows/") || lower === ".gitlab-ci.yml")
            add(facts.toolchains, "CI", path, sha);
    }
}
function sourceLanguageCounts(entries) {
    const counts = {};
    for (const entry of entries) {
        if (entry.type !== "file")
            continue;
...
function staticLiteral(node) {
    if (ts.isParenthesizedExpression(node))
        return staticLiteral(node.expression);
...
function staticModuleAssignment(node) {
    if (!ts.isBinaryExpression(node) || node.operatorToken.kind !== ts.SyntaxKind.EqualsToken)
        return false;
    if (!ts.isPropertyAccessExpression(node.left) || node.left.name.text !== "exports")
        return false;
