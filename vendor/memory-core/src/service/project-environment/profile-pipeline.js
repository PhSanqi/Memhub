...
export class ProjectEnvironmentProfilePipeline {
    ...
    async process(job, derived) {
        const payload = projectEnvironmentProfileJobPayload(job.payload);
        if (job.userId !== payload.userId)
            throw new Error("project_environment_job_owner_mismatch");
        const state = this.deps.repos.projectEnvironments.getState(payload.userId, payload.projectId);
        if (!state || state.currentScanId !== payload.scanId)
            return;
        const currentProfile = this.deps.repos.l3WorldModels.fields(payload.userId, payload.projectId).projectEnvironmentProfile;
        ...
        try {
            output = await completeStrictJson({
                ...
                validate: (value) => validateProjectEnvironmentProfileOutput(value, currentProfile, evidenceSupportsProfile)
            });
        }
        catch (error) {
            const latest = this.deps.repos.projectEnvironments.getState(payload.userId, payload.projectId);
            const latestProfile = this.deps.repos.l3WorldModels.fields(payload.userId, payload.projectId).projectEnvironmentProfile;
            if (!latest ||
...
export function projectEnvironmentProfileJobPayload(value) {
    ...
    if (!userId || !projectId || !scanId || (trigger !== "session_start" && trigger !== "token_compaction")) {
        throw new TypeError(`invalid project environment job payload: ${canonicalJson(value)}`);
    }
    return { userId, projectId, scanId, trigger };
}
export function validateProjectEnvironmentProfileOutput(value, currentProfile, evidenceSupportsProfile = false) {
    ...
    if (value.op === "noop" && value.profile !== "")
        throw new TypeError("noop profile must be empty");
    ...
    if (value.op === "update" && (currentProfile === null ||
...
function projectEnvironmentEvidenceSupportsProfile(derived) {
...
function profileDynamicInput(derived, currentProfile) {
    return {
