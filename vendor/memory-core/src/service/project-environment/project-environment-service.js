...
export class ProjectEnvironmentService {
    ...
    requestCompactionScan(session, throughTraceSeq) {
        if (!session.projectId)
            return null;
        const scope = this.deps.repos.l3WorldModels.getScope(session.userId, session.projectId);
        if (!scope?.workspaceUri)
    ...
    async processProfileJob(job) {
        ...
        if (!this.deps.repos.projectEnvironments.beginScan(payload.userId, payload.projectId, payload.scanId))
            return;
        try {
            const scope = this.deps.repos.l3WorldModels.getScope(payload.userId, payload.projectId);
            if (!scope?.workspaceUri)
                throw new Error("project_environment_workspace_uri_missing");
            ...
            if (state.fingerprint === derived.fingerprint &&
                state.appliedScanId &&
                provenanceMatches) {
            ...
            if (!this.deps.repos.projectEnvironments.markSummarizing(payload.userId, payload.projectId, payload.scanId))
                return;
