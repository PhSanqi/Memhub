...
import { resolveClaudeCodeHomeDirectory, resolveCodexHomeDirectory, resolveDeepseekHarnessHomeDirectory, resolveHermesHomeDirectory, resolveOpenclawStateDirectory, resolveOpencodeConfigDirectory, resolvePiAgentDirectory, resolveQwenworkHomeDirectory, resolveWorkbuddyHomeDirectory } from "./agent-paths.js";
...
import { createSourceRegistry } from "./adapters/source-registry.js";
import { isCompleteTurn, orderedTurns, renderTurnClipped, stableTurnIdentity, legacyTurnId, legacyTurnRequestId } from "../../../agent-source-core/index.js";
import { openMemoryAgentSourceScanStore } from "./scan-store.js";
...
import { createSkillTargetRegistry } from "./integration/target-registry.js";
...
export function createAgentSourceExecutor(options) {
    const registry = options.sourceRegistry ?? createBuiltinSourceRegistry();
    const configPath = options.configPath ?? join(process.env.MEMMY_HOME?.trim() || join(homedir(), ".memmy"), "config.yaml");
    ...
    let statePromise;
    let scan = emptyScanState();
    let scanTimer;
    let scanAbortController;
    let activeScanRequest;
    let scanPaused = false;
    let progressBeforePause = null;
    let resumePausedScan;
    let disposed = false;
    const activeScans = new Set();
    let activeAutomation;
    const readState = () => statePromise ??= loadState(statePath);
    const persist = async (state) => writeState(statePath, state);
    async function list() {
    ...
    async function startScan(input) {
        if (disposed)
            throw new MemoryServiceError("conflict", "Agent source executor is shutting down");
        const request = normalizeScanInput(input);
        if (scan.running)
            throw new MemoryServiceError("conflict", "An Agent source scan is already running");
        ...
        const activeScan = runScan(request, controller.signal, jobId).then(() => {
            ...
            if (scan.jobId !== jobId || controller.signal.aborted)
                return;
            scan = {
                ...scan,
                running: false,
    ...
    async function runScan(request, signal, jobId) {
        const failures = [];
        let failureCount = 0;
        ...
        let store;
        let completed = false;
        const preparedContentHashes = new Map();
        try {
            ...
            if (globalInitial)
                store.selectInitialTurns(stages.map((stage) => stage.sourceId), INITIAL_SCAN_MESSAGE_LIMIT, 200);
            for (const stage of stages) {
                await waitWhilePaused(signal);
                signal.throwIfAborted();
                const { adapter, stored, mode, sourceId, staged } = stage;
                if (mode === "initial_subset" && !globalInitial)
                    store.selectInitialTurns([sourceId], INITIAL_SCAN_MESSAGE_LIMIT, 0);
                ...
                const skillResult = await ingestAgentSkills(options.service, sourceId, store, options.resolveAgentSkillRoot, options.scheduleWorker);
            ...
            if (failureCount > 0) {
                ...
                if (meta)
                    store.saveMeta({ ...meta, phase: "failed", updatedAt: new Date().toISOString(), error: failures.slice(0, 3).join("; ") });
            }
            ...
            if (store) {
                const meta = store.getMeta();
                if (meta)
                    store.saveMeta({ ...meta, phase: "failed", updatedAt: new Date().toISOString(), error: error instanceof Error ? error.message : String(error) });
            }
            throw error;
        }
        finally {
    ...
    async function scanResults(jobId, cursor = "0", limit = 100) {
        const path = join(scanStoreDirectory, `${jobId}.sqlite`);
        if (!existsSync(path))
    ...
    async function mutateConnection(sourceId, kind, method) {
        ...
        if (!target)
            throw new MemoryServiceError("invalid_argument", `Agent source ${sourceId} cannot be connected automatically`);
        if (method === "POST") {
            ...
            if (kind === "plugin" && target.uninstallPlugin)
    ...
    function scheduleAutomation(delay, startup) {
        if (disposed)
            return;
        scanTimer = setTimeout(() => {
            ...
            activeAutomation = runAutomation(startup)
                ...
                .finally(() => scheduleAutomation(options.scheduledScanIntervalMs ?? SCHEDULED_SCAN_INTERVAL_MS, false));
        }, delay);
        scanTimer.unref?.();
    }
    async function runAutomation(startup) {
        if (disposed || scan.running)
            return;
        ...
        if (config.autoInjectSkill) {
            ...
            for (const source of discovered.sources) {
                if (disposed)
                    return;
                if (!source.available || source.status !== "not_connected")
                    continue;
                try {
    ...
    return {
        ...
        mutateConnection,
        startAutomation() {
            if (scanTimer || disposed)
                return;
            const config = loadMemmyConfig(configPath).config.agentAccess;
            scheduleAutomation(config.autoScanKnownAgents
                ? options.initialScanDelayMs ?? INITIAL_SCAN_DELAY_MS
                : options.scheduledScanIntervalMs ?? SCHEDULED_SCAN_INTERVAL_MS, config.autoScanKnownAgents);
        },
        async dispose() {
            disposed = true;
            if (scanTimer)
...
function findReusableScanJob(directory, sourceId, mode) {
    ...
    for (const name of readdirSync(directory).filter((value) => value.endsWith(".sqlite"))) {
        ...
        try {
            ...
            const row = db.prepare("SELECT job_id AS jobId, source_id AS sourceId, mode, phase, updated_at AS updatedAt FROM scan_meta WHERE id=1").get();
            db.close();
            if (row?.phase === "done") {
            ...
            if (row && row.sourceId === sourceId && (!mode || !row.mode || row.mode === mode)) {
                ...
                if (!selected || updatedAt > selected.updatedAt)
                    selected = { jobId: row.jobId, updatedAt };
            }
        }
...
function sameScanRequest(left, right) {
    return left.sourceId === right.sourceId && (right.mode === undefined || left.mode === right.mode);
}
export function createBuiltinSourceRegistry() {
...
export function createBuiltinIntegrationRegistry(configPath) {
...
async function stageStandaloneSource(adapter, stored, mode, store, signal, waitIfPaused, onProgress) {
        ...
        for await (const message of adapter.scan({
            ...
            if (bytes > 64 * 1024 * 1024) {
                ...
                if (errors.length < 1000)
                    errors.push(`${sourceId}:${message.conversationId}: ${reason}`);
...
async function ingestStagedMessages(service, store, sourceId, signal, onProgress, scheduleWorker) {
    let written = 0;
    let messageCount = 0;
    let processed = 0;
    ...
    const commitConversation = () => {
        if (!activeConversationId || activeConversationFailed)
            return;
    ...
    const flush = (force = false) => {
        if (memoryIds.length === 0 || (!force && memoryIds.length < 100))
            return;
    ...
    for await (const turn of orderedTurns(pages)) {
        ...
        if (turnLatest && (latestSeenAt === null || Date.parse(turnLatest) > Date.parse(latestSeenAt)))
            latestSeenAt = turnLatest;
        ...
        try {
            ...
            errorCount += 1;
            if (errors.length < 1000)
                errors.push(`${turn.conversationId}: ${reason}`);
...
async function prepareStandaloneSource(store, sourceId, mode, latestSeenAt, previousContentHash) {
    let cursor;
    ...
    const flushTurn = () => {
        if (!currentTurn.length || !isCompleteTurn(currentTurn))
            return;
    ...
    if (mode === "incremental" && previousContentHash !== contentHash)
...
function hashMeta(message, key) {
    const value = message.rawMeta[key];
    return typeof value === "string" ? value : undefined;
}
function readScanPage(store, sourceId, cursor) {
    const page = [];
    let bytes = 0;
    for (const message of store.messages(sourceId, cursor, 500)) {
        page.push(message);
        bytes += Buffer.byteLength(JSON.stringify(message));
        if (page.length >= 500 || bytes >= 8 * 1024 * 1024)
            break;
    }
    return page;
}
async function ingestAgentSkills(service, sourceId, store, resolveAgentSkillRoot = agentRootDirectory, scheduleWorker) {
    const root = resolveAgentSkillRoot(sourceId);
    if (!root)
        return { written: 0, memoryIdCount: 0, errorCount: 0, errors: [] };
    ...
    const pendingIds = [];
    const flush = (force = false) => {
        if (pendingIds.length === 0 || (!force && pendingIds.length < 100))
            return;
...
async function* findSkillFiles(root) {
    ...
    async function* visit(directory, depth) {
        ...
        for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
            if (entry.name === "memmy-memory" || entry.name === "node_modules" || entry.name === ".git")
                continue;
            const path = join(directory, entry.name);
            if (entry.isFile() && entry.name.toLowerCase() === "skill.md")
                yield path;
            else if (depth < 2 && entry.isDirectory())
                yield* visit(path, depth + 1);
        }
    }
}
function agentRootDirectory(sourceId) {
    switch (sourceId) {
...
function titleForTurn(sourceId, messages) {
...
async function loadState(path) {
    try {
        const parsed = JSON.parse(await readStateWithoutLegacyIds(path));
        ...
        if (value.version !== 2 || hasLegacyIds)
...
async function readStateWithoutLegacyIds(path) {
    ...
    try {
        for await (const chunk of input) {
            ...
            for (let index = 0; index < text.length; index += 1) {
                ...
                emitted += char;
                if (char === ":" && keyBuffer === '"importedRequestIds"')
                    pendingLegacyArray = true;
                if (!/\s/u.test(char))
                    keyBuffer = "";
            }
            if (emitted && !output.write(emitted))
