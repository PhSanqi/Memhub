...
export function createClaudeCodeSkillTarget(deps = {}) {
    ...
    return {
        ...
        async installPlugin(_targetId) {
            ...
            await writeFileAtomically(join(hookDirectory, HOOK_CONFIG_FILE_NAME), `${JSON.stringify({ memmy_config_path: memmyConfigPath, ...(await readMemmyMemoryServiceConfig(memmyConfigPath)) }, null, 2)}\n`);
            await writeFileAtomically(hookScriptPath, renderMemmyResumeHookScript({ source: CLAUDE_CODE_TARGET_ID, mode: "claude-code" }));
            await writeFileAtomically(join(hookDirectory, WORKSPACE_BRIDGE_FILE_NAME), await loadMemmyWorkspaceBridgeRuntimeAsset());
...
async function isSameFile(leftPath, rightPath) {
    try {
...
function removeLegacyMarkerBlock(existing) {
    return existing.replace(createMarkerBlockPattern(LEGACY_CLI_START_MARKER, LEGACY_CLI_END_MARKER), "");
}
async function upsertClaudeCodeHookSettings(filePath, hookScriptPath) {
...
async function removeClaudeCodeHookSettings(filePath) {
    const existing = await readTextFile(filePath);
    if (!existing.trim()) {
        return;
    }
...
function claudeHookEntries(value, hookScriptPath) {
    return [
...
function removeClaudeCodeResumeHookEntries(value) {
...
function isMemmyResumeHook(value) {
    if (!isRecord(value)) {
        return false;
    }
