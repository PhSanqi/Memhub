...
import { notifyRuntimeBoundary, openRuntimeSession, readRuntimeConfig, } from "./runtime.js";
    ...
    it("sends a compaction boundary only when Memory has an L1 head", async () => {
        ...
        const server = createServer(async (request, response) => {
            ...
            if (request.method === "GET")
                return json(response, 200, { throughL1MemoryId });
...
function runtimeSession(workspaceRoot, endpoint) {
    return {
        protocol: "v2",
        sessionId: "session-1",
        projectId: "project-1",
