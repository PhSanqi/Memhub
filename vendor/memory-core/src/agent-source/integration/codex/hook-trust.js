...
export async function trustMemmyCodexHooks(options) {
    ...
    try {
        ...
        const hooks = selectMemmyHooks(await listHooks(client, options.codexHomeDirectory), options.hooksFilePath, options.hookCommand);
...
async function listHooks(client, cwd) {
...
function selectMemmyHooks(hooks, hooksFilePath, hookCommand) {
    const sourcePath = normalize(hooksFilePath);
    const selected = hooks.filter((hook) => hook.source === "user" &&
        !hook.isManaged &&
...
function createJsonLineClient(child) {
    ...
    child.on("exit", (code, signal) => {
        if (!closing) {
            ...
            failPending(new Error(`Codex app-server exited before hook trust completed (${signal ?? code ?? "unknown"})${detail ? `: ${detail}` : ""}`));
        }
    });
    return {
...
function handleResponseLine(line, pending) {
    ...
    if (isRecord(message.error)) {
        request.reject(new Error(typeof message.error.message === "string" ? message.error.message : "Codex app-server request failed"));
        return;
    }
    request.resolve(message.result);
}
function resolveCodexExecutable(codexHomeDirectory) {
