...
export function createCodexSkillTarget(deps = {}) {
...
async function upsertCodexHookConfig(filePath, hookCommand) {
...
async function removeCodexHookConfig(filePath) {
    const existing = await readTextFile(filePath);
    if (!existing.trim()) {
        return;
    }
