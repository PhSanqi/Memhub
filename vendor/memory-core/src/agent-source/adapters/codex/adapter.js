...
export function createCodexSourceAdapter(deps = {}) {
    ...
    return {
        ...
        async *scan(options) {
            ...
            for (const [sessionIndex, session] of sessions.entries()) {
                ...
                for await (const rawMessage of streamConversationWindow(readCodexRollout(session.sessionFilePath, options.signal), options.since, options.signal, remainingMessageCapacity(options.maxMessages, emittedMessages), options.fullHistory)) {
                    throwIfAborted(options.signal);
...
function toConversationMessage(sourceId, rawMessage, workspacePath, gitRoot) {
    return {
