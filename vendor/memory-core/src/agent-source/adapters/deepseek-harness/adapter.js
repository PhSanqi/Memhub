...
import { streamDeepseekHarnessSession } from "./session-reader.js";
const SOURCE_ID = "deepseek_harness";
export function createDeepseekHarnessSourceAdapter(deps = {}) {
    ...
    return {
        ...
        async *scan(options) {
            ...
            for (const [sessionIndex, session] of sessions.entries()) {
                ...
                for await (const rawMessage of streamConversationWindow(streamDeepseekHarnessSession(session.sessionFilePath, options.signal), options.since, options.signal, remainingMessageCapacity(options.maxMessages, emittedMessages), options.fullHistory)) {
                    options.signal?.throwIfAborted();
                    emittedMessages += 1;
...
function toConversationMessage(message, gitRoot) {
    return {
        ...message,
        sourceId: SOURCE_ID,
