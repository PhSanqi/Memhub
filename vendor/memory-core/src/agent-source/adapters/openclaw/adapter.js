...
export function createOpenclawSourceAdapter(deps = {}) {
    ...
    return {
        ...
        async *scan(options) {
            ...
            })).filter((database) => database.schemaKind === "conversation" || database.schemaKind === "memory");
            options.onProgress?.({
            ...
            for (const [databaseIndex, database] of databases.entries()) {
                ...
                for await (const rawMessage of streamConversationWindow(readOpenclawDatabase(database.databasePath), options.since, options.signal, remainingMessageCapacity(options.maxMessages, emittedMessages), options.fullHistory)) {
                    throwIfAborted(options.signal);
                    options.onProgress?.({
                        sourceId: descriptor.sourceId,
                        phase: "redact",
