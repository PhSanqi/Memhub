...
export async function discoverOpenclawDatabases(options) {
    const files = await listSqliteFiles(options.root);
    const databases = files.map(readDatabaseCandidate).filter((database) => Boolean(database));
    return databases
...
function readDatabaseCandidate(databasePath) {
    try {
...
function isSqliteFileName(fileName) {
