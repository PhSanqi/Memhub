...
/** State db reader module. */
export async function* readHermesStateDb(path) {
    ...
    try {
        ...
        const sessionColumns = hasTable(db, "sessions") ? getTableColumns(db, "sessions") : new Set();
...
function toRawHermesStateDbMessage(row) {
...
function renderHermesMessageContent(role, row) {
    if (role !== "tool") {
...
function buildMessagesSql(messageColumns, sessionColumns) {
