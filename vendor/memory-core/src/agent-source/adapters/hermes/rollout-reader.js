...
/** Rollout reader module. */
export async function* readHermesRollout(filePath, signal) {
...
function toRawHermesMessage(record, fallbackConversationId, lineNumber) {
    ...
    const conversationId = getString(message.conversationId) ?? getString(record.conversationId) ?? getString(message.sessionId) ?? getString(record.sessionId) ?? fallbackConversationId;
    return {
...
function getMessageRecord(record) {
