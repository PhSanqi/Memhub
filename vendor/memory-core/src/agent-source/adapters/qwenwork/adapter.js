...
export function createQwenworkSourceAdapter(deps = {}) {
    ...
    return {
        ...
        async *scan(options) {
            ...
            for (const [sessionIndex, session] of sessions.entries()) {
                ...
                for await (const rawMessage of streamConversationWindow(readQwenworkHistory(session.sessionFilePath, options.signal), options.since, options.signal, remainingMessageCapacity(options.maxMessages, emittedMessages), options.fullHistory)) {
                    emittedMessages += 1;
