...
import { resolveWorkbuddyHomeDirectory, resolveWorkbuddyProjectsDirectory } from "../../agent-paths.js";
...
export function createWorkbuddySourceAdapter(deps = {}) {
    ...
    return {
        ...
        async *scan(options) {
            ...
            for (const [sessionIndex, session] of sessions.entries()) {
                ...
                for await (const rawMessage of streamConversationWindow(readWorkbuddyHistory(session.sessionFilePath, options.signal), options.since, options.signal, remainingMessageCapacity(options.maxMessages, emittedMessages), options.fullHistory)) {
                    throwIfAborted(options.signal);
                    options.onProgress?.({
                        sourceId: descriptor.sourceId,
                        phase: "redact",
...
function toConversationMessage(sourceId, rawMessage, discoveredWorkspacePath, gitRoot) {
    return {
