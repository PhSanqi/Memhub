...
export async function openMemoryAgentSourceScanStore(path, job) {
    ...
    if (!db.prepare("SELECT 1 FROM scan_meta WHERE id=1").get())
        db.prepare("INSERT INTO scan_meta(id,job_id,source_id,mode,phase,created_at,updated_at,error) VALUES(1,@jobId,@sourceId,@mode,@phase,@createdAt,@updatedAt,@error)").run({ ...job, error: job.error ?? null });
    let ordinal = Number(db.prepare("SELECT COALESCE(MAX(ordinal),-1) AS value FROM staged_messages WHERE job_id=?").get(job.jobId).value) + 1;
    ...
    const store = {
        path,
        stage(message) { const bytes = Buffer.byteLength(JSON.stringify(message)); if (bytes > MAX_RECORD_BYTES)
            throw new Error(`scan record exceeds 64 MiB limit (${bytes} bytes)`); return Number(insert.run(job.jobId, message.sourceId, message.conversationId, message.messageId, message.role, message.content, message.createdAt, message.workspacePath, message.gitRoot, JSON.stringify(message.rawMeta), ordinal++).changes) > 0; },
        stageBatch(messages) { const tx = db.transaction(() => { let count = 0; for (const message of messages)
            if (store.stage(message))
                count += 1; return count; }); return tx(); },
        messages(sourceId, cursor, limit = 500) { limit = Math.min(500, Math.max(1, limit)); const params = [job.jobId, sourceId]; let where = "job_id=? AND source_id=?"; if (cursor) {
            where += " AND ((conversation_id > ?) OR (conversation_id = ? AND (created_at > ? OR (created_at = ? AND (message_id > ? OR (message_id = ? AND ordinal > ?))))))";
            params.push(cursor.conversationId, cursor.conversationId, cursor.createdAt, cursor.createdAt, cursor.messageId, cursor.messageId, cursor.ordinal);
        } const rows = db.prepare(`SELECT source_id AS sourceId,conversation_id AS conversationId,message_id AS messageId,role,content,created_at AS createdAt,workspace_path AS workspacePath,git_root AS gitRoot,raw_meta_json AS rawMetaJson,ordinal FROM staged_messages WHERE ${where} ORDER BY conversation_id,created_at,message_id,ordinal LIMIT ?`).iterate(...params, limit); return (function* () { let bytes = 0; let count = 0; for (const row of rows) {
            const message = rowToMessage(row);
            yield message;
            count += 1;
            bytes += Buffer.byteLength(JSON.stringify(message));
            if (count >= 500 || bytes >= MAX_PAGE_BYTES)
                break;
        } })(); },
        ...
        getScanCursor(s) { return db.prepare("SELECT conversation_id AS conversationId,created_at AS createdAt,message_id AS messageId,ordinal FROM scan_cursors WHERE source_id=?").get(s) ?? null; },
        ...
        getSourceState(s) { return db.prepare("SELECT source_id AS sourceId,mode,phase,message_count AS messageCount,result_count AS resultCount,error_count AS errorCount,scan_started_at AS scanStartedAt,watermarked_since AS watermarkedSince,updated_at AS updatedAt,error FROM scan_source_state WHERE source_id=?").get(s) ?? null; },
        sourceCount() { return Number(db.prepare("SELECT COUNT(*) AS count FROM scan_source_state").get().count); },
        count(sourceId) { const row = db.prepare(`SELECT COUNT(*) AS count FROM staged_messages WHERE job_id=?${sourceId ? " AND source_id=?" : ""}`).get(job.jobId, ...(sourceId ? [sourceId] : [])); return Number(row.count); },
        ...
        getCheckpoint(s, c) { return db.prepare("SELECT source_id AS sourceId,conversation_id AS conversationId,last_message_id AS lastMessageId,last_created_at AS lastCreatedAt,content_hash AS contentHash,updated_at AS updatedAt FROM checkpoints WHERE source_id=? AND conversation_id=?").get(s, c) ?? null; },
        ...
        getConversationMeta(s, c) { const row = db.prepare("SELECT source_id AS sourceId,conversation_id AS conversationId,last_message_id AS lastMessageId,last_created_at AS lastCreatedAt,content_hash AS contentHash,selected FROM conversation_meta WHERE source_id=? AND conversation_id=?").get(s, c); return row ? { ...row, selected: row.selected === 1 } : null; }, selectAllConversations(s) { db.prepare("UPDATE conversation_meta SET selected=1 WHERE source_id=?").run(s); },
        ...
        getTurnMeta(s, c, t) { const row = db.prepare("SELECT source_id AS sourceId,conversation_id AS conversationId,turn_id AS turnId,first_message_id AS firstMessageId,first_created_at AS firstCreatedAt,last_message_id AS lastMessageId,last_created_at AS lastCreatedAt,selected FROM turn_meta WHERE source_id=? AND conversation_id=? AND turn_id=?").get(s, c, t); return row ? { ...row, selected: row.selected === 1 } : null; },
        ...
        getMeta() { return db.prepare("SELECT job_id AS jobId,source_id AS sourceId,mode,phase,created_at AS createdAt,updated_at AS updatedAt,error FROM scan_meta WHERE id=1").get() ?? null; },
        close() { db.close(); },
        remove() { db.close(); rmSync(path, { force: true }); rmSync(`${path}-wal`, { force: true }); rmSync(`${path}-shm`, { force: true }); }
    };
    return store;
}
function rowToMessage(row) { return { sourceId: String(row.sourceId), conversationId: String(row.conversationId), messageId: String(row.messageId), role: row.role, content: String(row.content), createdAt: String(row.createdAt), workspacePath: row.workspacePath == null ? null : String(row.workspacePath), gitRoot: row.gitRoot == null ? null : String(row.gitRoot), rawMeta: JSON.parse(String(row.rawMetaJson)), ordinal: Number(row.ordinal) }; }
