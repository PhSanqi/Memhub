...
export function retrievePluginMemories(input) {
    ...
    const selected = dedupeTraceEpisodeByEpisodeId(scoreSurvivors);
    const kept = {
...
export function hasMemoryRetrievalIndex(memory) {
    ...
    return hasVector(memoryVector(memory, "vec"));
}
...
function hasTaggedTraceVectorMatch(memories, layers, queryVec, options, traceVectorTags) {
    ...
    return memories.some((memory) => {
        ...
        if (!trace || !hasMemoryRetrievalIndex(memory))
            return false;
...
function candidateFromMemory(memory, queryVec, query, now, options) {
    ...
    const kind = memory.properties.internal_info.memory_kind === "span" ? "span" : kindFromLayer(memory.memoryLayer);
...
function readOnlyInjectionProfileOrDefault(value) {
    if (value === "experience") return "timeline";
    if (value === "skill_experience") return "skill_timeline";
    if (value === "timeline" || value === "skill" || value === "skill_timeline" || value === "all") return value;
...
function vectorChannelsForMemory(memory, queryVec, config, options = {}) {
    ...
    if (memory.memoryLayer === "L1") {
        ...
        remember(add("vec", memoryVector(memory, "vec"), config.minTraceSim));
    }
    return {
        channels,
        vector: best.vec,
