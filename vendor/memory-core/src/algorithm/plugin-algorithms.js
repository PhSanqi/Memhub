...
const RELATION_ALLOWED = ["revision", "follow_up", "new_task", "end_topic", "unknown"];
...
export function retrievalForIntent(kind) {
    switch (kind) {
        case "task":
...
export function classifyFeedbackText(raw) {
    const text = (raw ?? "").trim();
    if (!text) {
        return {
            shape: "unknown",
...
function noTurnFeedback(reason, method) {
    return {
        isFeedback: false,
        polarity: "neutral",
        magnitude: 0,
...
function detectFeedbackPreference(raw, normalized) {
...
function detectFeedbackCorrection(raw, normalized) {
...
function detectFeedbackConstraint(raw, _normalized) {
...
function cleanFeedbackClassifierText(value) {
    if (!value)
        return undefined;
    return value.trim().replace(/^["'`]|["'`]$/g, "").trim() || undefined;
}
function feedbackMatchesAny(value, patterns) {
    return patterns.some((pattern) => pattern.test(value));
}
function looksLikeFeedbackInstruction(raw, normalized) {
    const firstWord = (raw.match(/[A-Za-z\u4e00-\u9fff]+/)?.[0] ?? "").toLowerCase();
    if (FEEDBACK_IMPERATIVE_VERBS.has(firstWord))
        return true;
    return /\b(then|also|next)\s+(run|delete|create|install|try|use|call)\b/.test(normalized);
}
const FEEDBACK_PREFERENCE_PATTERNS = [
...
const FEEDBACK_CORRECTION_PATTERNS = [
...
const FEEDBACK_CONSTRAINT_PATTERNS = [
...
const FEEDBACK_CONFUSION_PATTERNS = [
...
function matchIntentHeuristic(text) {
    ...
    if (/\b(what did (i|we) (say|discuss|talk|mention)|do you remember|last time|earlier we|previously we|we talked about)\b/i.test(text) ||
...
function intentDecision(kind, confidence, reason, signals) {
    return {
        kind,
...
export async function classifyTurnRelationWithLlm(input, options = {}) {
    const text = input.newUserText.trim();
    if (!text) {
        return relationDecision("unknown", 0, "empty message", ["empty"]);
    }
    ...
    if (!options.disableLlm && llm?.isConfigured()) {
        try {
            const result = await relationWithTimeout(callRelationLlm(llm, input), options.timeoutMs ?? 6000);
            if (result.relation === "new_task" && result.confidence < RELATION_ARBITRATION_THRESHOLD) {
                try {
                    const arbitration = await relationWithTimeout(callRelationArbitration(llm, input), options.timeoutMs ?? 6000);
...
function collectTurnRelationRules(input, gapMs) {
    const text = input.newUserText.trim();
    const rules = [];
...
function safeHeuristicRelation(decision) {
    if (decision.relation !== "new_task" || decision.confidence >= RELATION_ARBITRATION_THRESHOLD) {
        return decision;
    }
    return relationDecision("follow_up", 0.55, "weak new-task signal; defaulting to follow_up", [...decision.signals, "weak_new_task_fallback"]);
}
...
async function callRelationLlm(llm, input) {
    ...
    if (typeof value.relation !== "string" || !RELATION_ALLOWED.includes(value.relation)) {
...
async function callRelationArbitration(llm, input) {
    const value = await llm.completeJson([
...
function relationUserPrompt(input) {
...
function relationArbitrationUserPrompt(input) {
    return [
...
async function relationWithTimeout(promise, timeoutMs) {
    let timer;
    try {
...
export const RETRIEVAL_QUERY_EXTRACT_PROMPT = {
    ...
    version: 2,
    description: "Extract semantic, lexical, and optional time-range constraints for memory retrieval.",
...
export const SKILL_CRYSTALLIZE_PROMPT = {
    ...
    version: 6,
    description: "Turn graduated L2 evidence into a callable SOP-style skill with retrieval-oriented metadata.",
...
export const SKILL_REBUILD_PROMPT = {
    id: "skill.rebuild",
    version: 3,
    description: "Update an existing SOP skill from new evidence while preserving stable identity and controlling rewrite scope.",
...
export function combineRewardAxes(input) {
    return clamp(0.45 * clamp(input.goalAchievement, -1, 1) +
        0.3 * clamp(input.processQuality, -1, 1) +
        0.25 * clamp(input.userSatisfaction, -1, 1), -1, 1);
}
export function retrievalLayersForMode(mode = "search") {
    switch (mode) {
        case "tool_driven":
        case "sub_agent":
            return ["L2", "L1", "L3"];
...
export function isResearchDomain(domain) {
    return domain === "research";
}
export function effectiveReadOnlyInjectionProfile(config) {
    if (!isResearchDomain(config.domain))
        return "all";
    return config.readOnlyInjectionProfile ?? "all";
}
export function retrievalLayersForProfile(layers, config) {
...
export const BATCH_REFLECTION_PROMPT = {
    id: "reflection.batch",
    version: 13,
    description: "Tri-valued path-relevance scoring for each step in an episode window.",
...
export function buildPluginRetrievalQuery(ctx) {
    switch (ctx.reason) {
        case "turn_start": {
...
export function extractResearchQuestionSection(raw) {
    const text = raw.trim();
    if (!text)
        return null;
    ...
    for (let index = start; index < lines.length; index += 1) {
        ...
        if (body.length > 0 && /^#{1,3}\s+\S/.test(line.trim()))
            break;
        body.push(line);
    }
...
export function focusResearchRetrievalQuery(raw, domain) {
    const text = raw.trim();
...
export function extractRetrievalTags(text) {
...
export function isStandaloneMathFinalAnswerTask(text) {
    if (!text)
        return false;
    const normalized = text.toLowerCase();
    if (isCodeGenerationTask(normalized))
        return false;
    const mathSignals = [
...
function isCodeGenerationTask(normalized) {
    ...
    if (explicitCodeContract.some((re) => re.test(normalized)))
        return true;
    ...
    const hasProgrammingSurface = /\b(?:code|program|python|javascript|typescript|java|c\+\+|function|method|class|stdin|stdout|standard input|standard output)\b/.test(normalized);
    return hasProgrammingSurface && structuralSignals >= 2;
}
export function renderMathFinalAnswerProtocol(text) {
    void text;
    return [
...
export function isRepositoryRepairPrompt(text) {
    const raw = String(text ?? "");
    return hasRepositoryRepairDescription(raw) && hasRepositoryRepairIntent(raw);
}
function extractRepositoryRepairQueryText(text) {
    if (!hasRepositoryRepairDescription(text))
        return null;
    if (!hasRepositoryRepairIntent(text))
        return null;
...
function hasRepositoryRepairDescription(text) {
    return /##\s*(?:Issue|Bug) Description\b/i.test(text);
}
function hasRepositoryRepairIntent(text) {
...
function extractRepositoryName(text) {
    const patterns = [
...
function extractRepositoryRepairDescription(text) {
    return extractRepairTaskSection(text, "Issue Description") ||
        extractRepairTaskSection(text, "Bug Description");
}
export function extractRepairTaskSection(text, heading) {
    const escapedHeading = heading.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const match = text.match(new RegExp(`(?:^|\\n)\\s*##\\s*${escapedHeading}\\b\\s*([\\s\\S]*?)(?=\\n\\s*(?:##\\s+|STRICT RULES:|Reply\\s+[A-Z_]+\\b)|$)`, "i"));
    return (match?.[1] ?? "").trim();
}
export function renderRepositoryRepairProtocol(text) {
    ...
    protocol.push("", "### Patch-first completion contract", "1. The goal is a small non-empty behavior-changing source `git diff`, not an explanation, comment-only change, test-only change, or proof that the bug already appears fixed.", "2. Use read-only commands to locate the target, but do not exceed eight inspect/search commands before the first source edit. If evidence points to one function/class, write the minimal tentative patch with the exact-replacement script, then test and inspect `git diff`.", "3. If visible bug or hint context appears above, treat it as the current task action queue: grep one exact issue identifier, inspect the containing source once or twice, then edit the candidate source behavior before searching tests broadly.", "4. Edit-readiness rule: after you have inspected the candidate function/class named by the current prompt, the next tool call should create an outer-write `/tmp/memmy_edit.py` exact-replacement script for the smallest source patch. Do not inspect tests first.", "5. When the next action would be another broad grep, another nearby `sed`, or a test search, prefer writing `/tmp/memmy_edit.py` for the smallest source behavior change already supported by the prompt and current source.", "6. If `git diff` is empty, the task is not complete.", "7. If a just-run targeted test, reproduction, or assertion fails after your patch, do not declare completion; inspect the failing expected/actual output and revise the patch until the targeted check passes or the failure is clearly unrelated.", "", "### Hard gates", "1. Stay in the repository selected by the current task wrapper. Never switch to another repository directory unless the current prompt explicitly says so.", `2. Use the exact current wrapper reference \`${runtime.wrapperRef}\` from the task prompt. Do not reuse a hard-coded path from memory; retries can change it.`, `3. Use copy-paste-safe wrapper calls matching the task prompt: double quotes around the \`${runtime.runVerb}\` command, then single quotes inside that command when a search pattern needs quoting.`, `   - Inspect: \`${shellPrefix}\``, `   - Good identifier grep: \`${renderRunCommand(runtime, "grep -n target_symbol path/to/file.py")}\`. Search one bare identifier, not a phrase with spaces.`, `   - Good literal grep: \`${renderRunCommand(runtime, "grep -n 'literal_pattern_without_spaces' path/to/file.py")}\``, `   - Good sed: \`${renderRunCommand(runtime, "sed -n '120,180p' path/to/file.py")}\``, `   - Good poll: \`${renderRunCommand(runtime, "", "10")}\` only after a previous command reported still running.`, `   - Good script/status/diff: \`${renderRunCommand(runtime, `python ${runtime.editScriptPath}`)}\`, \`${renderRunCommand(runtime, "git diff")}\`, \`${renderRunCommand(runtime, "git status --porcelain")}\`.`, `4. Never put raw inner double quotes inside the \`${runtime.runVerb} "..."\` command. Bad: \`grep -n "pattern" file\`; good: \`grep -n 'pattern' file\`.`, runtime.repoRoot
        ...
        : `5. Do not invent a repository root. If the wrapper starts in the repository, run commands directly; if you later confirm a root with \`pwd\`, use \`cd <that-root> &&\` consistently for \`${runtime.runVerb}\` commands.`, `6. Do not use complex shell forms inside \`${runtime.runVerb}\`: no inline \`python - <<\`, no heredoc, no \`cat >\`, no \`tee\`, no \`sed -i\`, no \`perl -pi\`, no \`apply_patch\`, no \`patch\`, no \`git apply\`, no \`sh -lc\`, no \`bash -lc\`, no shell pipes (\`|\`), no nested \`${runtime.runVerb}\`, and no empty \`${runtime.runVerb}\` except polling a still-running command.`, `7. Never grep for a phrase containing whitespace inside \`${runtime.runVerb}\` such as \`grep -n "def target_symbol"\`, \`grep -n '^    def target_symbol'\`, or \`rg -n "def target_symbol"\`; those can split wrapper arguments or leave stale running commands. Grep the bare identifier (\`target_symbol\`) or inspect a line range with \`nl\`/\`sed\`.`, `8. Use outer \`write\` for target files or temporary scripts, e.g. \`${writePrefix}\`. The wrapper is host-side, so do not inspect it from the repository or call it inside \`${runtime.runVerb}\`.`, `9. A \`${runtime.wrapperRef} write\` heredoc is literal. Do not put shell substitutions like \`$(sed ...)\`, command output placeholders, line numbers, or diff markers (\`+\`/\`-\` prefixes) into the file content.`, `10. If any \`${runtime.runVerb}\` result starts with a bare \`>\` prompt or only echoes lines like \`> command\`, the shell is stuck in quote/heredoc continuation. Stop normal commands, interrupt first${runtime.interruptCommand ? ` with \`${renderControlCommand(runtime, runtime.interruptCommand, "3")}\`` : ""}, then confirm a normal prompt before running scripts or \`git diff\`.`, "11. Do not finish by saying the issue is already fixed. For repository repair tasks, a valid completion needs a non-empty source `git diff`; if `git diff` is empty, continue source investigation.", "", "### Safe edit loop", `1. For any multi-line source edit, first create \`${editScriptPrefix}\` with the outer wrapper.`, `2. The edit script should use \`Path('${renderTargetPath(runtime, "path/to/file")}')\`, exact \`old\`/\`new\` replacement strings copied from inspected source, \`assert old in text\`, and \`text.replace(old, new, 1)\`.`, `3. Run the script with \`${renderRunCommand(runtime, `python ${runtime.editScriptPath}`)}\`.`, "4. If `OLD block not found`, inspect the actual block with simple `nl`/`sed`/single-pattern `grep`, then rewrite the temporary edit script with outer `write`. Do not fall back to inline heredoc, `sed -i`, or broad rewrites.", "5. If a command syntax error mentions an unclosed quote/heredoc/bracket, stop using that command shape immediately and switch to the outer-write temporary script pattern.", `6. If a poll or script run repeats stale source text from an earlier command instead of showing the new command output, stop polling. Run one fresh \`${renderRepoCommand(runtime, "git diff -- <target-file>")}\` or \`${renderRepoCommand(runtime, "git status --porcelain")}\`; if that also repeats stale text, rewrite the edit/check script and execute it with the same repository command prefix.`, "", "### Search and test discipline", "1. Prefer POSIX tools (`grep`, `find`, `nl`, `sed`) because `rg` may be unavailable. Use simple single-token searches; avoid shell pipelines (`|`), phrase searches with spaces, and alternation (`\\|`) in wrapper command strings because host command parsers and allowlists are often conservative.", "2. If a Repair hint context or Visible issue context is present, inspect the target source file at most twice, then apply the minimal source edit before searching for regression-test locations.", "3. If repair hints contain a candidate source diff or visible issue clues contain a current -> expected expression, apply the minimal source fix before running full tests or searching broadly. Existing tests come after the source edit.", "4. Source behavior determines task success. Do not create or keep searching for new regression tests after existing targeted tests pass; run `git diff` and finish.", "5. When a candidate diff is present, do not generalize the same idea to other similar call sites, files, tests, docs, or helper functions unless the candidate diff explicitly touches them. Extra edits outside the candidate hunks can break existing behavior checks.", "6. Verify with the project's native targeted tests. Only use a generic test runner after confirming the project already uses it; do not install a new test runner just for one repair.", "7. A command that reports zero tests found, zero tests run, a missing runner, or an import/configuration error is not a passing verification; either run the correct native target or rely on a direct reproduction that exercises the changed source.", `8. Before declaring completion${runtime.completionToken ? ` with \`${runtime.completionToken}\`` : ""}, run \`git diff\`, confirm the patch is non-empty, and check it does not delete unrelated files or tests.`, "9. If the latest targeted test/reproduction output contains `FAIL`, `ERROR`, `AssertionError`, an expected-vs-actual mismatch, or the same original output after your edit, treat that output as the next edit target; a non-empty patch alone is not enough to finish.", "10. If `git diff` is empty, tests or reproduction output are not enough to finish. Keep narrowing the source behavior and make the smallest source edit.", "11. If the Repair hint context contains a required `+` checklist or expected added-line count, it is a completion gate: compare `git diff` against it and continue editing until every listed source line/effect is present. Targeted or broad tests passing is not sufficient when the checklist is incomplete.", "12. Convergence budget: after locating the visible target class/function and one neighboring same-family implementation, either write the minimal source patch or run one narrow reproduction. Do not keep doing broad grep/test searches without producing a patch.", "13. If generic repair heuristics are present, use them as a short source-inspection checklist only; patch the current repository behavior, not the heuristic text.", "14. If a Visible issue context names identifiers, do not run `ls` or `pwd` first; the first command should grep one exact issue identifier.");
    return protocol.join("\n");
}
function inferRepairRuntime(text) {
    const raw = String(text ?? "");
    const wrapperMatch = raw.match(/^\s*([A-Z][A-Z0-9_]*(?:_PATH|_WRAPPER|_HANDLE)?)\s*[:=]\s*(\/\S+|\S+)/im) ??
...
function extractPromptRepoRoot(text) {
...
function renderVisibleRepairContext(text, runtime) {
    ...
    if (identifiers.length === 0 && replacements.length === 0)
        return null;
...
const GENERIC_DEFECT_HEURISTICS = [
    {
...
function renderGenericDefectContext(text) {
    const source = [
    ...
    if (!source.trim())
        return null;
    const matched = selectGenericDefectHeuristics(GENERIC_DEFECT_HEURISTICS.filter((heuristic) => heuristic.re.test(source)));
    if (matched.length === 0)
        return null;
    return truncateHintDigest([
...
function selectGenericDefectHeuristics(matched) {
...
function extractRepairIdentifiers(text) {
    const buckets = [
    ...
    const seen = new Set();
    const out = [];
    const addToken = (raw, source = "generic") => {
    ...
    for (const match of text.matchAll(/\b([A-Z][A-Za-z0-9]*)\/([A-Z][A-Za-z0-9]+)\b/g)) {
        ...
        if (rightParts && OPERATION_PREFIXES.includes(left)) {
...
function normalizeRepairIdentifier(token) {
    return token
        .replace(/[.,;:)\]]+$/g, "")
        .replace(/^[([{\s]+/g, "")
        .trim();
}
function isUsefulRepairIdentifier(token) {
    if (token.length < 3 || token.length > 120)
        return false;
    if (/^https?:/i.test(token))
        return false;
    if (/^(?:Bug|Issue|Description|Patch|Reply|Done)$/i.test(token))
        return false;
    if (/\s/.test(token))
        return false;
...
function isUsefulRepairCallIdentifier(token) {
...
function renderOrderedExampleGuidance(callAnchors) {
    if (callAnchors.length < 2)
        return "";
    return [
...
function extractOrderedCallAnchors(text) {
    const seen = new Set();
    const out = [];
    for (const match of text.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\s*\(/g)) {
        const token = normalizeRepairIdentifier(match[1] ?? "");
        if (!isUsefulRepairCallIdentifier(token))
            continue;
...
function splitOperationToken(token) {
...
function extractInsteadOfPairs(text) {
    const pairs = [];
...
function cleanPromptExpression(value) {
...
function renderRepairHintContext(text, runtime) {
...
function renderAnchorGuidance(anchors, runtime) {
    if (anchors.length === 0)
        return "";
...
function renderAnchorSearchCommand(anchor) {
...
function extractImplementationAnchors(text) {
    const seen = new Set();
    const out = [];
    const add = (raw) => {
        const token = normalizeRepairIdentifier(String(raw ?? ""));
        if (!isUsefulImplementationAnchor(token))
            return;
        const key = token.toLowerCase();
        if (seen.has(key))
            return;
...
function isUsefulImplementationAnchor(token) {
    if (token.length < 3 || token.length > 120)
        return false;
    if (/^https?:/i.test(token))
        return false;
...
function stripDiffPathPrefix(token) {
    return String(token ?? "").replace(/^(?:a|b)\//, "");
}
function renderExactReplacementScriptPattern(target, runtime) {
    return [
...
function extractFirstDiffTarget(diffText) {
    const match = diffText.match(/\+\+\+\s+b\/([^\s]+)/);
    return match?.[1]?.trim() || null;
}
function truncateHintDigest(text, maxChars = 1_600) {
    if (text.length <= maxChars)
        return text;
    return `${text.slice(0, maxChars - 3).trimEnd()}...`;
}
function formatUnifiedDiffForHint(diffText) {
...
const INLINE_REFLECTION_PATTERNS = [
...
const DEFAULT_RETRIEVAL_TUNING = {
    tier1TopK: 3,
    tier2TopK: 5,
    tier3TopK: 2,
...
export function captureTurnSteps(input) {
    ...
    })).filter((step) => step.userText.length > 0 ||
...
export function traceMetaFromMemory(memory) {
    if (memory.memoryLayer !== "L1")
        return null;
    const trace = getInternal(memory, "trace");
    if (!trace)
        return null;
...
export function displayReflectionText(reflection) {
    const text = reflection?.trim() ?? "";
    if (!text)
        return null;
    return INTERNAL_REFLECTION_LABELS.has(text.toUpperCase()) ? null : text;
}
export function policyMetaFromMemory(memory) {
    if (memory.memoryLayer !== "L2")
        return null;
    const policy = getInternal(memory, "policy");
    if (!policy)
        return null;
    ...
    return {
        ...
        sourceFeedbackIds: distinct([
            ...
            ...stringArrayField(memory.properties.internal_info, "source_feedback_ids")
        ]),
        decisionGuidance: {
...
export function policyRequiresRevalidation(policy, now = Date.now()) {
    if (policy.freshnessClass !== "dynamic")
        return false;
    if (!policy.revalidateAfter)
        return true;
    const revalidateAt = Date.parse(policy.revalidateAfter);
    return !Number.isFinite(revalidateAt) || revalidateAt <= now;
}
export function policyIsEligibleForDownstream(policy, now = Date.now()) {
    return policy.status === "active" && !policyRequiresRevalidation(policy, now);
}
export function failureAvoidancePolicyIsRetrievalEligible(policy) {
    if (policy.experienceType !== "failure_avoidance" && policy.evidencePolarity !== "negative") {
        return true;
    }
    if (policy.confidence < 0.6 || !policy.trigger.trim())
        return false;
    ...
    if (preferences.size === 0 || antiPatterns.size === 0)
        return false;
    return [...preferences].some((item) => !antiPatterns.has(item));
}
function normalizeGuidanceForComparison(value) {
    return value
        .toLowerCase()
...
export function skillMetaFromMemory(memory) {
    ...
    const procedure = recordField(skill, "procedure_json") ??
        recordField(memory.properties.internal_info, "procedure_json");
    return {
        id: memory.id,
        memory,
        ...
        successRate: numberField(skill, "success_rate") ?? successRate(numberField(skill, "trials_attempted") ?? 0, numberField(skill, "trials_passed") ?? 0),
        betaPosterior: betaPosteriorField(skill, "beta_posterior") ?? betaPosterior(numberField(skill, "trials_attempted") ?? 0, numberField(skill, "trials_passed") ?? 0),
        vec: memoryVector(memory, "vec")
    };
}
export function worldModelMetaFromMemory(memory) {
    if (memory.memoryLayer !== "L3")
        return null;
    const wm = getInternal(memory, "world_model");
    if (!wm)
        return null;
    ...
    if (v2Fields) {
        ...
        const sourceMemoryIds = stringArrayField(memory.properties.internal_info, "source_memory_ids");
        return {
            id: memory.id,
            memory,
            title: project ? "项目场域认知" : "通用规则与安全约束",
            domainKey: project ? `project:${memory.info.project_id}` : "general:no_project",
    ...
    return {
        ...
        summary: stringField(wm, "summary") ?? stringField(memory.properties.internal_info, "summary"),
        body: stringField(wm, "body") ?? memory.memoryValue,
        vec: memoryVector(memory, "vec")
    };
}
function l3WorldModelV2Fields(memory, worldModel) {
    ...
    if (Object.keys(worldModel).sort().join(",") !== expectedKeys.join(","))
        return null;
    const value = (key) => {
...
export function retrievalDocumentForMemory(memory) {
...
export function retrievalDocumentSourceHash(memory) {
    return stableHash(retrievalDocumentForMemory(memory));
}
export function retrievalDocumentIsCurrent(memory) {
    const index = recordField(memory.properties.internal_info, "retrieval_index");
...
function worldModelTitleFromMemory(memory, wm) {
    return firstWorldModelDisplayString(stringField(wm, "title"), firstReadableWorldModelLine(memory.memoryValue), isInternalMemoryKey(memory.memoryKey) ? undefined : memory.memoryKey, memory.id) ?? memory.id;
}
function firstWorldModelDisplayString(...values) {
    return values
        .map(cleanWorldModelDisplayText)
        .find((value) => Boolean(value && !isWorldModelSectionHeading(value) && !isInternalMemoryKey(value)));
}
function firstReadableWorldModelLine(value) {
    return value
        .split(/\r?\n/)
        .map(cleanWorldModelDisplayText)
        .find((line) => Boolean(line && !isWorldModelSectionHeading(line) && !isInternalMemoryKey(line)));
}
function cleanWorldModelDisplayText(value) {
    const text = (value ?? "")
...
function isWorldModelSectionHeading(value) {
...
function worldModelStructureField(record, key) {
...
function worldModelStructureEntries(value) {
...
export function buildPolicyDraft(args) {
    const support = distinct(args.evidenceTraces
    ...
    if (status === "active" && minSupport >= 3) {
        const successfulEpisodes = distinct(args.evidenceTraces
            .filter((trace) => trace.value > 0)
            .map((trace) => trace.episodeId || trace.id)
            .filter(isString)).length;
        if (successfulEpisodes < 2)
            status = "candidate";
    }
...
export function detectDominantLanguage(samples) {
    let zh = 0;
    let en = 0;
...
export function languageSteeringLine(language) {
    switch (language) {
        case "zh":
...
export function packL2InductionTraces(traces, charCap, signatureLabelText) {
...
function formatTraceTools(calls) {
...
export function buildWorldModelDraft(args) {
    ...
    const eligible = args.policies.filter((policy) => policy.status === "active" &&
    ...
    for (const [domainKey, bucket] of buckets) {
        ...
        const body = [
            ...
            `Source policy signals: ${cohort.map((policy) => [policy.title, policy.trigger, policy.procedure]
                .filter(Boolean)
                .join(" / ")).join(" ; ")}`,
...
export function shapeWorldModelConfidence(baseConfidence, admission, cohesion) {
...
export function buildSkillDraft(args) {
    if (args.policy.status !== "active")
        return null;
    const minEta = args.minEtaForRetrieval ?? 0.1;
    const minSupport = args.minSupport ?? 1;
    const minGain = args.minGain ?? 0.02;
    if (args.policy.gain < minGain)
        return null;
    if (args.policy.support < minSupport)
        return null;
    if (!hasPolicySuccessAnchor(args.policy))
        return null;
    const existing = args.existing ?? null;
    const eta = existing && existing.trialsAttempted > 0
        ? clamp01(existing.eta)
        : clamp01(Math.max(minEta, 0.5 * clamp01(args.policy.gain) + 0.5 * Math.min(1, args.policy.support / Math.max(1, minSupport))));
...
function successRate(attempted, passed) {
    if (attempted <= 0)
        return 0;
    return clamp01(passed / attempted);
}
function betaPosterior(attempted, passed) {
...
function betaPosteriorField(record, key) {
    const raw = recordField(record, key);
    if (!raw)
        return undefined;
    ...
    if (alpha === undefined || beta === undefined || mean === undefined)
        return undefined;
    return {
        alpha,
        beta,
...
export function hasPolicySuccessAnchor(policy) {
    if (!policy.skillEligible)
        return false;
...
export function skillEtaAfterTrial(input) {
    ...
    const priorEta = clamp01(input.currentEta * (priorStrength + previousAttempts) - previousPasses);
...
export function heuristicHumanScore(feedback) {
    ...
    for (const item of scored) {
        ...
        if (item.polarity === "neutral" || magnitude === 0)
            continue;
...
export function backpropagateTraces(input) {
    ...
    const weights = new Array(traces.length).fill(0);
    let sumW = 0;
    ...
    const updates = new Array(traces.length);
    for (let index = 0; index < traces.length; index += 1) {
...
function alphaFromTraceReflection(trace) {
    const reflection = trace.reflection?.trim();
    if (!reflection) {
        const alpha = clamp(trace.alpha, 0, 1);
        return alpha > 0 ? alpha : 0.5;
    }
    if (reflection === "IRRELEVANT")
        return 0;
    if (reflection === "RELATED" || reflection === "RELATED_DEFAULT")
        return 0.5;
    if (reflection === "PIVOTAL")
        return 1;
    return clamp(trace.alpha, 0, 1);
}
export function priorityFor(value, ts, decayHalfLifeDays = 30, now = Date.now()) {
...
export function retrievePluginMemories(input) {
    ...
    const traceVectorTagsRequired = traceVectorTags.length > 0 &&
        (config.tagFilter === "on" || hasTaggedTraceVectorMatch(input.memories, layers, queryVec, {
            mode,
    ...
    const candidates = input.memories
        ...
        .map((memory) => candidateFromMemory(memory, queryVec, compiledQuery, now, {
        mode,
        excludeTraceRawTurnIds: input.excludeTraceRawTurnIds,
    ...
    const survivors = mode !== "decision_repair" && requiresKeywordConfirmation(compiledQuery.text)
        ? thresholdSurvivors.filter((candidate) => bypassesKeywordConfirmation(candidate) || hasKeywordChannel(candidate))
        : thresholdSurvivors;
    ...
    const selected = suppressFeedbackExperiencesCoveredBySkills(dedupeTraceEpisodeByEpisodeId(scoreSurvivors));
    const kept = {
...
export function centroid(vectors) {
    const present = vectors.filter((vector) => Array.isArray(vector) && vector.length > 0);
    if (present.length === 0)
        return null;
...
export function signatureFromTrace(trace) {
    return trace.signature || signatureFromTraceLike(trace.tags, trace.toolCalls, trace.reflection ?? "");
}
export function signatureFromTraceParts(tags, toolCalls, reflection = "") {
    return signatureFromTraceLike(tags, toolCalls, reflection);
}
export function l2CandidateSignatureHash(signature) {
    let hash = 5381;
...
function normalizeToolCalls(toolCalls, toolResults) {
...
function extractInlineReflection(agentText) {
    if (!agentText)
        return null;
...
export function extractErrorSignatures(step) {
    const candidates = new Map();
    for (const call of step.toolCalls) {
        const errorCode = errorCodeFromToolCall(call);
        if (errorCode)
            addErrorFragment(candidates, errorCode);
...
function signatureFromTraceLike(tags, toolCalls, reflection) {
...
function firstErrCode(toolCalls, reflection) {
...
function countSharedSignatureComponents(left, right) {
    let count = 0;
    if (left.primaryTag !== "_" && left.primaryTag === right.primaryTag)
        count += 1;
    if (left.secondaryTag !== "_" && left.secondaryTag === right.secondaryTag)
        count += 1;
    if (left.tool !== "_" && left.tool === right.tool)
        count += 1;
    return count;
}
function isDistinctSignatureComponent(left, right) {
    if (!left || !right)
        return false;
    if (left === "_" || right === "_")
        return false;
...
function addErrorFragment(candidates, fragment) {
...
function normalizeErrorFragment(fragment) {
    const collapsed = fragment
        .replace(/\s+/g, " ")
        .replace(/[\u200b\u00a0]/g, "")
        .trim();
    if (collapsed.length < MIN_ERROR_FRAGMENT_LEN)
        return null;
    ...
    if (words.length > 0 && words.every((word) => ERROR_STOP_WORDS.has(word)))
        return null;
    return truncated;
}
function errorSpecificityScore(fragment, frequency) {
    let score = 0;
    if (/\b[A-Z][a-zA-Z]*Error\b/.test(fragment))
        score += 3;
    if (/\b[A-Z][a-zA-Z]*Exception\b/.test(fragment))
        score += 3;
    if (/(\b|_)E[A-Z]{3,}\b/.test(fragment))
        score += 2;
    if (/\/[a-zA-Z0-9._\-/]+/.test(fragment))
        score += 2;
    if (/\bcode\s*=\s*\d+/.test(fragment))
        score += 1;
    if (/\b\d{3}\b/.test(fragment))
        score += 1;
    if (/_/.test(fragment))
        score += 1;
...
function buildPolicyProcedure(evidence) {
...
function fallbackWorldModelSummary(title, structure) {
    const facts = [
...
function skillNameFromPolicy(policy) {
...
function renderInvocationGuide(policy, name) {
    return [
        `# ${name}`,
        "",
        policy.procedure,
...
function toolsFromSignature(signature) {
    const parts = signature.split("|");
    const tool = parts[2];
    return tool && tool !== "_" ? [tool] : [];
}
function skillDraftTools(draft) {
...
export function extractToolNamesFromTraces(traces) {
    const out = new Set();
    for (const trace of traces) {
        for (const call of trace.toolCalls) {
            const name = call.name?.trim().toLowerCase();
            if (name && name !== "unknown" && name !== "unknown_tool")
                out.add(name);
...
function skillEvidenceResonance(draft, evidenceTraces) {
...
export function isMemoryReadyForRetrieval(memory) {
    if (memory.status === "deleted" || memory.status === "archived")
        return false;
    const evidenceStatus = memory.properties.internal_info.evidence_status;
    if (evidenceStatus === "provisional" || evidenceStatus === "disputed")
        return false;
    const policy = memory.memoryLayer === "L2" ? policyMetaFromMemory(memory) : null;
    if (policy && policy.status !== "candidate" && policy.status !== "active")
        return false;
    if (policy && policyRequiresRevalidation(policy))
        return false;
    const skill = memory.memoryLayer === "Skill" ? skillMetaFromMemory(memory) : null;
    if (skill && skill.status !== "candidate" && skill.status !== "active")
        return false;
    if (hasMemoryRetrievalIndex(memory))
        return true;
    if (hasPendingImportPipeline(memory))
        return false;
    return hasSearchableText(memory);
}
export function hasMemoryRetrievalIndex(memory) {
    if (memory.status === "deleted" || memory.status === "archived")
        return false;
...
function hasRetrievalEmbedding(memory, meta) {
    if (memory.memoryLayer === "L1") {
...
function hasSearchableText(memory) {
    return typeof memory.memoryValue === "string" && memory.memoryValue.trim().length > 0;
}
function hasPendingImportPipeline(memory) {
...
function hasTaggedTraceVectorMatch(memories, layers, queryVec, options, traceVectorTags) {
    if (!layers.includes("L1") || queryVec.length === 0 || traceVectorTags.length === 0)
        return false;
    return memories.some((memory) => {
        ...
        if (memory.status === "deleted" || memory.status === "archived")
            return false;
        if (!memoryHasAnyTag(memory, traceVectorTags))
            return false;
        const trace = traceMetaFromMemory(memory);
        if (!trace || !hasRetrievalEmbedding(memory, { trace, policy: null, skill: null, world: null }))
            return false;
        if (shouldExcludeTraceFromTurnContext(trace, options))
            return false;
        ...
        }).channels.some((channel) => channel.channel === "vec_summary" || channel.channel === "vec_action");
    });
}
function memoryHasAnyTag(memory, tags) {
    if (tags.length === 0)
        return true;
...
function candidateFromMemory(memory, queryVec, query, now, options) {
    if (memory.status === "deleted" || memory.status === "archived")
        return null;
    const trace = memory.memoryLayer === "L1" ? traceMetaFromMemory(memory) : null;
    const policy = memory.memoryLayer === "L2" ? policyMetaFromMemory(memory) : null;
    const skill = memory.memoryLayer === "Skill" ? skillMetaFromMemory(memory) : null;
    const world = memory.memoryLayer === "L3" ? worldModelMetaFromMemory(memory) : null;
    if (!isMemoryReadyForRetrieval(memory))
        return null;
    if (trace && shouldExcludeTraceFromTurnContext(trace, options))
        return null;
    if (memory.memoryLayer === "Skill" &&
    ...
    const vectorChannels = vectorChannelsForMemory(memory, queryVec, options.config, {
        suppressTraceVector: memory.memoryLayer === "L1" &&
    ...
    if (memory.memoryLayer === "Skill") {
        if (!skill || !["active", "candidate"].includes(skill.status))
            return null;
        if (skill.eta < options.config.minSkillEta)
            return null;
    }
    ...
    if (ftsScore > 0)
        channels.push({ channel: "fts", score: ftsScore, rawScore: ftsScore, rank: 0 });
    if (patternScore > 0)
        channels.push({ channel: "pattern", score: patternScore, rawScore: patternScore, rank: 0 });
    if (structuralScore > 0)
        channels.push({ channel: "structural", score: structuralScore, rawScore: structuralScore, rank: 0 });
...
function shouldExcludeTraceFromTurnContext(trace, options) {
    return Boolean(trace.rawTurnId && options.excludeTraceRawTurnIds?.has(trace.rawTurnId));
}
function suppressFeedbackExperiencesCoveredBySkills(candidates) {
    const coveredPolicyIds = new Set();
    const coveringSkills = new Map();
    ...
    if (coveredPolicyIds.size === 0)
        return candidates;
    return candidates.filter((candidate) => {
        if (candidate.memory.memoryLayer !== "L2")
            return true;
...
function limitCandidatePoolByTier(candidates, config) {
    const caps = {
...
function compareCandidatePrePool(a, b) {
    return b.relevance - a.relevance ||
        b.channels.length - a.channels.length ||
        b.vectorScore - a.vectorScore;
}
function addEpisodeRollupCandidates(candidates, config, now) {
    const topTraceIds = new Set(candidates
    ...
    for (const candidate of candidates) {
        if (candidate.memory.memoryLayer !== "L1")
            continue;
        if (!topTraceIds.has(candidate.memory.id))
            continue;
    ...
    for (const [episodeId, bucket] of byEpisode.entries()) {
        ...
        if (representative.vectorScore < config.episodeGoalMinSim)
            continue;
        if ((representativeTrace?.value ?? 0) < 0 && config.episodeGoalMinSim > 0)
            continue;
        const ordered = [...bucket].sort((a, b) => (traceMetaFromMemory(a.memory)?.ts ?? 0) - (traceMetaFromMemory(b.memory)?.ts ?? 0));
        ...
        const bestChannelScore = Math.max(representative.vectorScore, ...channels.map((channel) => channel.score), 0);
        rollups.push({
            ...representative,
            id: episodeId,
...
function compareEpisodeRollupTrace(a, b) {
...
function dedupeCandidateChannels(channels) {
    const best = new Map();
...
function renderEpisodeRollupSnippet(traces, totalTraceCount) {
    ...
    const steps = traces.slice(0, 6).flatMap((trace, index) => {
        ...
        if (reflection)
            parts.push(`reflection: ${reflection.slice(0, 160)}`);
...
function assignCandidateChannelRanks(candidates) {
    const channels = ["vec", "vec_summary", "vec_action", "fts", "pattern", "structural"];
    const tiers = ["tier1", "tier2", "tier3"];
    for (const tier of tiers) {
        for (const channel of channels) {
            const ranked = candidates
                .filter((candidate) => candidate.tier === tier && candidate.channels.some((item) => item.channel === channel))
                .sort((a, b) => (b.channels.find((item) => item.channel === channel)?.rawScore ??
...
function recomputeCandidateRelevance(candidates, config, now) {
    for (const candidate of candidates) {
...
function assignChannelRrf(candidates, rrfConstant) {
    for (const candidate of candidates) {
        candidate.rrf = candidate.channels.reduce((sum, channel) => sum + 1 / (rrfConstant + channel.rank + 1), 0);
    }
}
function dedupeTraceEpisodeByEpisodeId(candidates) {
    ...
    for (const group of groups.values()) {
        ...
        if (winner.source === "episode") {
            for (const candidate of group.traces)
...
function compareTraceEpisodeCandidates(a, b) {
...
function requiresKeywordConfirmation(text) {
...
function hasKeywordChannel(candidate) {
    return candidate.channels.some((channel) => channel.channel === "fts" ||
        channel.channel === "pattern" ||
        channel.channel === "structural");
}
function bypassesKeywordConfirmation(candidate) {
    return candidate.memory.memoryLayer === "Skill" || candidate.memory.memoryLayer === "L3";
}
function retrievalTuning(input) {
    return {
...
function readOnlyInjectionProfileOrDefault(value) {
    if (value === "experience" ||
        value === "skill" ||
...
function priorityBlendFor(config) {
    if (config.weightPriority <= 0)
        return 0;
...
function mmrSelect(candidates, limit, config) {
    ...
    for (const tier of tiers) {
        ...
        for (let index = 0; index < pool.length; index += 1) {
            ...
            if (candidate.tier !== tier)
                continue;
            if (candidate.relevance > tierBestRelevance)
                tierBestRelevance = candidate.relevance;
            if (config.smartSeed && candidate.relevance < seedCutoff)
                continue;
        ...
        if (bestIndex < 0)
            continue;
        if (tierBestRelevance < seedCutoff)
            continue;
        ...
        if (candidate.vector)
            selectedVectors.push(candidate.vector);
    }
...
function mmrCandidateScore(candidate, selectedVectors, config) {
...
function vectorChannelsForMemory(memory, queryVec, config, options = {}) {
    const channels = [];
    const add = (channel, vec, floor) => {
        const candidateVec = vec ?? null;
        const seededScore = options.seededChannelScores?.[channel];
        const score = Math.max(0, seededScore ?? (options.useSeededChannelScores ? 0 : cosine(queryVec, candidateVec)));
        if (candidateVec && score >= floor) {
            channels.push({ channel, score, rawScore: score, rank: 0 });
        }
        return { score, vec: candidateVec };
    };
    let best = { score: 0, vec: null };
    const remember = (item) => {
        if (item.vec && (!best.vec || item.score > best.score))
            best = item;
    };
    if (memory.memoryLayer === "L1") {
...
function memoryTextForRetrieval(memory) {
    const trace = traceMetaFromMemory(memory);
    if (trace) {
        return [
            trace.summary,
...
function memoryFtsTextForRetrieval(memory) {
    const policy = policyMetaFromMemory(memory);
    if (policy) {
...
function snippetForCandidateMemory(memory, retrievalText) {
    const trace = traceMetaFromMemory(memory);
    if (trace) {
        return clampLength([
            trace.summary,
...
function ftsChannelScore(terms, text) {
    if (terms.length === 0)
        return 0;
    const lower = text.toLowerCase();
...
function patternChannelScore(terms, text) {
    if (terms.length === 0)
        return 0;
    const lower = text.toLowerCase();
...
function structuralOverlap(fragments, memory, text) {
...
export function compileRetrievalQuery(raw, extract, opts = {}) {
    return finalizeRetrievalQuery(focusResearchRetrievalQuery(raw, opts.domain).text, extract);
}
function finalizeRetrievalQuery(raw, extract) {
...
function normalizeRetrievalExtract(raw, extract) {
    const fallback = fallbackRetrievalExtract(raw);
    if (!extract)
        return { queryText: fallback.queryVecText, keywords: fallback.keywords };
...
export function fallbackRetrievalExtract(raw) {
...
function expandCjkPreferenceKeywords(text) {
...
function normalizePromptText(raw) {
    ...
    if (repositoryRepairPrompt)
        return repositoryRepairPrompt;
    return text;
}
function sanitizeRetrievalKeywords(keywords) {
...
function isUsableRetrievalQueryText(text) {
...
function extractKeywordTokens(text) {
    ...
    for (const token of tokens) {
        ...
        if (GENERIC_STOP_WORDS.has(normalized))
            continue;
...
function prepareMemoryFtsMatch(text) {
...
function extractMemoryFtsTerms(text) {
    if (!text)
        return [];
...
function extractMemoryPatternTerms(text) {
    if (!text)
        return [];
...
function renderRetrievalArgs(args) {
    if (!args)
        return "";
    try {
...
function retrievalHintText(hints) {
    if (!hints)
        return "";
...
function renderRetrievalHintValue(value) {
...
function getInternal(memory, key) {
...
function booleanishField(record, key) {
    const value = record[key];
...
function vectorField(record, key) {
    const value = record[key];
    if (!Array.isArray(value))
        return null;
    const vector = value.filter((item) => typeof item === "number" && Number.isFinite(item));
...
function statusField(record, key, allowed) {
    const value = record[key];
...
function errorMessageFromUnknown(value) {
    if (!value)
        return undefined;
    if (typeof value === "string" && /error|exception|failed|exit\s*[1-9]/i.test(value))
        return value;
...
function clampUnknown(value, max) {
    if (typeof value === "string")
...
function relationDecision(relation, confidence, reason, signals) {
    return {
        relation,
...
function relationMatchesAny(text, patterns) {
    return patterns.some((pattern) => pattern.test(text));
}
function relationQuotesPreviousAssistant(newText, prevAssistantText) {
    ...
    for (let index = 0; index + 8 <= words.length; index += 1) {
        ...
        if (window.length > 24 && lower.includes(window))
            return true;
    }
    return false;
}
function relationDomainShift(newText, prevTags) {
...
function relationTiePriority(decision) {
    if (decision.signals.includes("r5_new_phrase"))
        return 40;
    if (decision.signals.includes("r1_negation_keyword"))
        return 30;
    if (decision.signals.includes("r2_quotes_prev"))
        return 20;
    if (decision.signals.includes("r3_pronoun_ref"))
        return 10;
    return 0;
}
