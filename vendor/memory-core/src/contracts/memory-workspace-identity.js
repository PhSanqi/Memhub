...
export function normalizeWorkspaceUri(input) {
...
export function deriveWorkspaceHostId(installationId) {
    if (!installationId.trim())
        throw new TypeError("installationId must be non-empty");
...
function isLocalFileSystemRoot(url) {
    if (!LOCAL_HOST_NAMES.has(url.hostname.toLowerCase()))
        return false;
