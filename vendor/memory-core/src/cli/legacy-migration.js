...
export async function migrateLegacyLocalPlugins(options) {
    ...
    if (dataSources.length === 0) {
        if (configSource)
            await importLegacyConfig(configSource, options.configPath);
        return report;
    }
    ...
    try {
        ...
        const run = target.transaction(() => {
            for (const source of dataSources)
                report.sources.push(importLegacyDatabase(target, source));
        });
        run();
    }
    finally {
...
export function discoverLegacySources(root = homedir()) {
    const userRoot = resolve(root);
    return Object.entries(LEGACY_HOMES).map(([agent, relative]) => {
...
async function selectConfigSource(candidates, options) {
    if (candidates.length === 0)
        return undefined;
    if (options.configSource) {
        const selected = candidates.find((candidate) => candidate.agent === options.configSource);
        if (!selected)
            throw new Error(`--config-source ${options.configSource} was requested but no legacy config was found`);
        return selected;
    }
...
async function importLegacyConfig(source, targetPath) {
    const parsed = parseYaml(await readFile(source.configPath, "utf8"));
...
function mergeLegacyAlgorithm(current, legacy) {
    const result = structuredClone(current);
    for (const section of ["capture", "reward", "feedback", "l2Induction", "l3Abstraction", "skill", "session", "retrieval"]) {
        if (Object.keys(record(legacy[section])).length)
            result[section] = { ...record(result[section]), ...record(legacy[section]) };
    }
    return result;
}
function importLegacyDatabase(target, source) {
...
function importOlderLegacyDatabase(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
    ...
    for (const chunk of chunks) {
        ...
        const rawTurnId = mapAndInsert(target, source, "chunks:raw_turn", sourceId, "raw_turns", rawRow, maps, inserted, deduplicated, remapped);
        const trace = {
    ...
    return { agent: source.agent, database: source.dbPath, inserted, deduplicated, remapped };
    function ensureSession(sourceId, timestamp) {
    ...
    function ensureChunkEpisode(sessionKey, turnId, sessionId, timestamp) {
...
function importTraces(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
    for (const row of rows(legacy, "traces")) {
        ...
        const tags = jsonArray(row.tags_json).filter((tag) => typeof tag === "string");
...
function importPolicies(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
...
function importWorldModels(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
    for (const row of rows(legacy, "world_model")) {
        ...
        const memory = memoryRow({
            ...
            title, body, tags: worldModel.domain_tags.filter((tag) => typeof tag === "string"),
...
function importSkills(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
    for (const row of rows(legacy, "skills")) {
        const sourceId = requiredString(row.id, "skills.id");
...
function importFeedback(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
...
function importTracePolicyLinks(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
...
function importSkillTrials(target, legacy, source, maps, userId, inserted, deduplicated, remapped) {
...
function importHubState(target, legacy, source, inserted, deduplicated) {
...
function finalizeEpisodes(target, legacy, maps) {
...
function mapAndInsert(target, source, sourceTable, sourceId, targetTable, row, maps, inserted, deduplicated, remapped) {
    ...
    WHERE source_path = ? AND source_table = ? AND source_id = ? AND content_sha256 = ?`)
        .get(source.dbPath, sourceTable, sourceId, contentDigest);
    if (ledger?.target_id) {
    ...
    if (targetTable === "raw_turns" && typeof row.session_id === "string" && typeof row.turn_id === "string") {
        const existingRawTurn = target.prepare(`SELECT id, user_text, assistant_text, reasoning_summary,
              tool_calls_json, tool_results_json, source_memory_ids_json,
              usage_json, message_payload_json
       FROM raw_turns
       WHERE session_id = ? AND turn_id = ?`).get(row.session_id, row.turn_id);
        if (existingRawTurn) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, 'deduplicated', ?)
      `).run(source.dbPath, sourceTable, sourceId, contentDigest, targetTable, existingRawTurn.id, new Date().toISOString());
    ...
    const sameMemory = targetTable === "memories"
        ? target.prepare("SELECT id FROM memories WHERE content_hash = ? AND memory_layer = ? LIMIT 1").get(row.content_hash, row.memory_layer)
        : undefined;
    if (sameMemory?.id) {
        ...
        if (targetId !== sourceId)
            remapped[`${sourceTable}:${sourceId}`] = targetId;
    }
    if (!sameMemory) {
...
function mergeRawTurn(target, existing, incoming) {
    ...
    WHERE id = ?
  `).run(mergeText(existing.user_text, incoming.user_text), mergeText(existing.assistant_text, incoming.assistant_text), mergeText(existing.reasoning_summary, incoming.reasoning_summary), json(mergeJsonArrays(existing.tool_calls_json, incoming.tool_calls_json)), json(mergeJsonArrays(existing.tool_results_json, incoming.tool_results_json)), json(mergeJsonArrays(existing.source_memory_ids_json, incoming.source_memory_ids_json)), json({ ...jsonRecord(existing.usage_json), ...jsonRecord(incoming.usage_json) }), json(payload), existing.id);
}
...
function insertRow(target, table, row) {
    const columns = target.prepare(`PRAGMA table_info(${table})`).all().map((item) => String(item.name));
...
function uniqueTargetId(target, table, column, agent, sourceId, contentDigest) {
    const base = `legacy_${agent}_${sanitizeId(sourceId)}_${contentDigest.slice(0, 10)}`;
    let candidate = base;
    let suffix = 1;
    while (target.prepare(`SELECT 1 FROM ${table} WHERE ${column} = ?`).get(candidate))
...
function mapped(maps, targetTable, sourceId) {
    return maps[targetTable]?.get(sourceId) ?? sourceId;
}
function optionalMapped(maps, targetTable, sourceId) {
    return sourceId ? mapped(maps, targetTable, sourceId) : null;
}
function setMap(maps, targetTable, sourceId, targetId) {
    (maps[targetTable] ??= new Map()).set(sourceId, targetId);
    if (targetTable === "memories")
        (maps.memories ??= new Map()).set(sourceId, targetId);
    if (targetTable === "raw_turns")
        (maps.raw_turns ??= new Map()).set(sourceId, targetId);
}
function increment(target, key) { target[key] = (target[key] ?? 0) + 1; }
function requiredString(value, field) { const result = string(value); if (!result)
    throw new Error(`legacy database field is missing: ${field}`); return result; }
function string(value) { return typeof value === "string" && value.trim() ? value.trim() : undefined; }
function nullableString(value) { return string(value) ?? null; }
function finite(value) { return typeof value === "number" && Number.isFinite(value) ? value : null; }
function record(value) { return value && typeof value === "object" && !Array.isArray(value) ? value : {}; }
function json(value) { return JSON.stringify(value); }
function jsonValue(value, fallback) { if (typeof value !== "string")
    return value ?? fallback; try {
    return JSON.parse(value);
}
catch {
    return fallback;
} }
function jsonRecord(value) { return record(jsonValue(value, {})); }
function jsonArray(value) { const parsed = jsonValue(value, []); return Array.isArray(parsed) ? parsed : []; }
function sqlValue(value) { return value === undefined || value === null ? null : Buffer.isBuffer(value) ? value : typeof value === "number" || typeof value === "string" ? value : json(value); }
function iso(value) { const numeric = finite(value); const date = numeric === null ? new Date() : new Date(numeric); return Number.isFinite(date.getTime()) ? date.toISOString() : new Date().toISOString(); }
...
function stableJson(value) { if (Array.isArray(value))
    return `[${value.map(stableJson).join(",")}]`; if (value && typeof value === "object" && !Buffer.isBuffer(value))
    return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${stableJson(value[key])}`).join(",")}}`; if (Buffer.isBuffer(value))
    return JSON.stringify(value.toString("base64")); return JSON.stringify(value) ?? "null"; }
function sanitizeId(value) { return value.replace(/[^0-9A-Za-z_.-]+/g, "_").slice(0, 48) || "item"; }
function compact(value) { return Object.fromEntries(Object.entries(value).filter(([, item]) => item !== undefined)); }
function redactHubSecrets(row) { const next = { ...row }; for (const key of ["token_hash", "user_token", "api_key", "apiKey"])
    if (key in next)
        next[key] = "[REDACTED]"; return next; }
