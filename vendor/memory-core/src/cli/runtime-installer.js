...
export async function installMemoryRuntime(options = {}) {
    ...
    if (!descriptor)
        throw new Error(`Memory release ${manifest.version} does not support ${target}`);
    ...
    const previous = await readJsonFile(currentPath);
    ...
    try {
        ...
        else if (repairLegacyTask)
            tryRepairLegacyWindowsTaskRegistration(home, serviceHome);
        if (!options.skipHealthCheck) {
            try {
                await waitForRuntimeHealth(options.endpoint ?? "http://127.0.0.1:18960", manifest.version, healthCheckTimeoutMs);
            }
            catch (error) {
...
export async function currentInstalledRuntime(home = "~/.memmy") {
    return readJsonFile(join(resolveHome(home), "memory-service", "current.json"));
}
export async function installedAgents(home = "~/.memmy") {
    const installation = await readJsonFile(join(resolveHome(home), "memory-service", "installation.json"));
    return Array.isArray(installation?.agents)
        ? installation.agents.filter((agent) => typeof agent === "string" && agent.length > 0)
        : [];
}
export async function startInstalledMemoryService(home = "~/.memmy") {
    ...
    try {
        ...
        if (!pointer)
            throw new Error("Memory is not installed");
...
export async function repairInstalledWindowsMemoryService(home = "~/.memmy") {
    const resolvedHome = resolveHome(home);
    ...
    const installLock = await acquireInstallLock(join(serviceHome, "install.lock"));
    try {
        if (!isLegacyWindowsTask(resolvedHome))
            return { ok: true, repaired: false };
        const pointer = await currentInstalledRuntime(resolvedHome);
        if (!pointer)
            throw new Error("Memory is not installed");
...
export function userServiceRestartCommand(platform = process.platform, uid = process.getuid?.() ?? 0) {
    if (platform === "darwin") {
        return {
            command: "launchctl",
...
export function restartInstalledMemoryService() {
...
export async function stopInstalledMemoryService(home = "~/.memmy", dependencies = {}) {
    const resolvedHome = resolveHome(home);
    const runtimeState = await readJsonFile(join(resolvedHome, "memory-service", "runtime.json"));
...
async function probeMemoryRuntime(endpoint, headers, request) {
    try {
...
async function waitForMemoryRuntimeStop(endpoint, headers, request) {
    const deadline = Date.now() + SERVICE_STOP_TIMEOUT_MS;
    while (Date.now() < deadline) {
        if (await probeMemoryRuntime(endpoint, headers, request) === "stopped")
            return true;
...
async function reuseInstalledRuntime(pointer, home, serviceHome, options, healthCheckTimeoutMs) {
    if (options.dryRun)
        return { ok: true, reused: true, dryRun: true, ...pointer };
    const installLock = await acquireInstallLock(join(serviceHome, "install.lock"));
    try {
        ...
        if (process.platform === "win32" && (!options.skipServiceRegistration || repairLegacyTask))
            await stopInstalledMemoryService(home);
        ...
        if (!options.skipServiceRegistration)
            ...
            tryRepairLegacyWindowsTaskRegistration(home, serviceHome);
        if (!options.skipHealthCheck) {
            await waitForRuntimeHealth(options.endpoint ?? "http://127.0.0.1:18960", pointer.version, healthCheckTimeoutMs);
        }
...
async function resolveReleaseManifest(options, target) {
    ...
    if (options.runtimeDirectory) {
        ...
        const metadata = await readJsonFile(join(path, "memory-runtime.json"));
    ...
    const parsed = JSON.parse(await readSourceText(manifestUrl));
...
function parseReleaseManifest(value) {
    ...
    return { version: value.version, protocolVersion: value.protocolVersion, assets };
}
async function obtainRuntimeAsset(options, manifest, descriptor, destination) {
    if (options.runtimeAsset) {
...
async function validateRuntime(path, version, target, protocolVersion) {
    const manifest = await readJsonFile(join(path, "memory-runtime.json"));
    ...
    if (!existsSync(entrypoint))
        throw new Error(`Memory runtime entrypoint is missing: ${entrypoint}`);
}
...
async function writeStableLauncher(home, serviceHome, nodeExecutable) {
...
function isLegacyWindowsTask(home) {
    if (process.platform !== "win32")
        return false;
...
function tryRepairLegacyWindowsTaskRegistration(home, serviceHome) {
    try {
...
function registerAndStartUserService(home, serviceHome, start = true) {
...
function runLifecycle(command, args, allowFailure = false) {
...
async function waitForRuntimeHealth(endpoint, expectedVersion, timeoutMs) {
...
async function removeUserServiceRegistration() {
    if (process.platform === "darwin") {
        runLifecycle("launchctl", ["bootout", `gui/${process.getuid?.() ?? 0}/com.memtensor.memmy-memory`], true);
...
function resolveHealthCheckTimeoutMs(value) {
    if (value === undefined)
...
function resolveHome(value) { return resolve(value === "~" ? homedir() : value.startsWith("~/") ? join(homedir(), value.slice(2)) : value); }
...
function systemdEscape(value) { return value.replace(/([\\"\s])/g, "\\$1"); }
function xmlEscape(value) { return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;"); }
function mkdirSyncForLifecycle(path) { mkdirSync(path, { recursive: true }); }
function writeFileSyncForLifecycle(path, value, encoding = "utf8") { writeFileSync(path, value, { encoding, mode: 0o600 }); }
