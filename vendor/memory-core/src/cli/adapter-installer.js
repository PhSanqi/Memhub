...
import { normalizeAgentIds } from "./skill-writer/index.js";
export async function installAgentAdapters(options) {
    ...
    for (const agent of agents) {
        ...
        if (!options.dryRun && !existsSync(source))
            throw new Error(`Memory ${options.runtime.version} is missing its ${agent} adapter`);
        if (agent === "openclaw")
            results.push(await installOpenClaw(source, root, options));
        if (agent === "hermes")
            results.push(await installHermes(source, root, options));
        if (agent === "dsh")
            results.push(await installDsh(source, root, options));
    }
    return results;
}
async function installOpenClaw(source, root, options) {
    ...
    if (!existsSync(openClawRoot) && !options.explicit)
        return result("openclaw", target, false, false, options);
    if (!existsSync(openClawRoot) && options.explicit)
        throw new Error(`openclaw is not installed: ${openClawRoot}`);
    if (!options.dryRun) {
        ...
        if (options.restartHosts !== false)
            runOptional("openclaw", ["gateway", "restart"]);
    }
    return result("openclaw", target, true, true, options);
}
async function installHermes(source, root, options) {
    const hermesRoot = process.env.HERMES_HOME?.trim() || join(root, ".hermes");
    const target = join(hermesRoot, "plugins", "memmy");
    if (!existsSync(hermesRoot) && !options.explicit)
        return result("hermes", target, false, false, options);
    if (!existsSync(hermesRoot) && options.explicit)
        throw new Error(`hermes is not installed: ${hermesRoot}`);
    if (!options.dryRun) {
        await replaceDirectory(source, target);
...
async function installDsh(source, root, options) {
    const dshRoot = process.env.DSH_HOME?.trim() || join(root, ".dsh");
    const target = join(dshRoot, "profiles", options.dshProfile ?? "web");
    if (!existsSync(dshRoot) && !options.explicit)
        return result("dsh", target, false, false, options);
    if (!existsSync(dshRoot) && options.explicit)
        throw new Error(`dsh is not installed: ${dshRoot}`);
    if (!options.dryRun) {
        const installed = spawnSync("dsh", ["plugin", "--profile", options.dshProfile ?? "web", "add", source], { encoding: "utf8", windowsHide: true });
        if (installed.status !== 0)
            throw new Error(`failed to install DSH adapter: ${installed.stderr?.trim() || installed.stdout?.trim() || installed.error?.message}`);
    }
...
function result(agent, target, installed, configured, options) {
