import { hasOption, isRecord, optionBoolean, optionString, optionValues, parseArgs, parseStringArray, readBodyObject, requireValue } from "./args.js";
import { sendRequest } from "./http.js";
import { renderCliOutput } from "./render/index.js";
import { initMemoryCli, installMemoryCli, upgradeMemoryCli } from "./setup.js";
import { DEFAULT_MEMORY_URL, loadCliMemoryConfig } from "./config.js";
import { PROJECT_VERSION } from "./project-version.js";
import { currentInstalledRuntime, repairInstalledWindowsMemoryService, startInstalledMemoryService, stopInstalledMemoryService } from "./runtime-installer.js";
...
export async function runCommand(context) {
    ...
    if (words[0] === "service") {
        const home = optionString(options, "home") ?? "~/.memmy";
        if (words[1] === "start")
            return startInstalledMemoryService(home);
        if (words[1] === "stop")
            return (context.stopInstalledService ?? stopInstalledMemoryService)(home);
        if (words[1] === "repair-launcher")
            return (context.repairInstalledService ?? repairInstalledWindowsMemoryService)(home);
        if (words[1] === "status")
            return { ok: true, runtime: await currentInstalledRuntime(home) ?? null };
...
async function mapTopLevelCommand(words, parsed) {
    const [group] = words;
    switch (group) {
        case "health":
...
async function reloadConfigRequest(parsed) {
    return {
        method: "POST",
...
async function sessionsRequest(action, args, parsed) {
    switch (action) {
        case "open":
            return {
                method: "POST",
...
async function turnsRequest(action, args, parsed) {
    switch (action) {
        case "start": {
...
async function buildMemorySearchRequest(args, parsed) {
    const query = optionString(parsed.options, "query") ?? args[0];
    return {
        method: "POST",
        path: "/memory/search",
...
async function addMemoryRequest(args, parsed) {
...
function getMemoryRequest(args, parsed) {
...
async function deleteMemoryRequest(args, parsed) {
    const id = optionString(parsed.options, "id") ?? args[0];
    return {
        method: "DELETE",
        path: `/memory/${encodeURIComponent(requireValue("id", id))}`
...
async function runRaw(words, parsed, options) {
...
async function serveMemory(parsed) {
...
function compactMemoryGetOutput(result) {
...
function memoryDetailRecord(result) {
    if (!isRecord(result))
        return undefined;
    if (isMemoryDetailLike(result))
        return result;
    const item = result.item;
    return isRecord(item) && isMemoryDetailLike(item) ? item : undefined;
}
function isMemoryDetailLike(value) {
    return Boolean(stringValue(value.id) && (stringValue(value.body) ||
...
function compactMemoryGetContent(result, detail) {
    const rawTurn = rawTurnRecord(result);
    if (rawTurn)
        return compactRawTurnContent(rawTurn);
    return compactMemoryBody(stringValue(detail.body) || stringValue(detail.content));
}
function compactRawTurnContent(rawTurn) {
    const parts = [];
...
function compactMemoryBody(body) {
...
function isInternalMemoryBodyLine(line, hasConversation) {
    if (/^\s*(RawTurn|TraceStep|Alpha|Value|Priority):/i.test(line))
        return true;
    return hasConversation && /^\s*Summary:/i.test(line);
}
function compactRawTurnToolDetails(rawTurn) {
    if (!rawTurn)
        return undefined;
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        if (input)
            lines.push(...toolFieldLines("input", input));
        const output = toolOutput(call, resultRecord);
        if (output)
            lines.push(...toolFieldLines("output", output));
        const error = toolError(call, resultRecord);
        if (error)
            lines.push(...toolFieldLines("error", error));
    }
    return lines.join("\n");
}
function rawTurnRecord(result) {
...
function toolResultFor(call, results, index) {
...
function toolName(call, result, index) {
...
function toolFieldLines(label, value) {
    return value.includes("\n")
        ? [`  ${label}:`, ...value.split("\n").map((line) => `    ${line}`)]
        : [`  ${label}: ${value}`];
}
async function requestBody(parsed, fields, normalize) {
...
function requestOptions(parsed, fetchImpl) {
    const configPath = optionString(parsed.options, "config");
...
function withSource(request, parsed) {
...
function positiveIntegerOption(parsed, name) {
    if (!hasOption(parsed.options, name))
        return undefined;
...
function stringArrayOption(parsed, name) {
    const value = optionString(parsed.options, name);
    if (value === undefined)
        return undefined;
