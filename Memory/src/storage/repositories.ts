...
function buildMemoryWhere(filter: MemoryFilter): { where: string; params: SqlValue[] } {
  ...
  function addProjectScopeClause(projectIds: string[] | undefined, includeUnscoped: boolean | undefined): void {
    if (projectIds === undefined && includeUnscoped === undefined) return;
    ...
    const clausesForScope: string[] = [];
    if (includeUnscoped === true) clausesForScope.push(`${projectExpression} IS NULL`);
