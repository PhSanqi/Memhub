...
`memhub_distill action=discover` reconciles completed, Memory-Core-ingested, project-resolved L1 captures into idempotent L2 jobs. `dry_run=true` reports candidates without enqueueing. The same reconciler runs after complete HTTP/MCP capture and on the idle timer. It groups by account, canonical project and conversation; a single complete turn can qualify after the idle window. Existing jobs, including failed jobs, count as already queued and are not silently retried. A queueing failure is reported separately from successful L1 ingestion, and a later discovery pass can recover it.
For older clients, an empty `next` also attempts discovery when the policy is enabled, then retries leasing. A discovery failure is reported separately and does not discard an existing queued job.

Enabling the automatic policy records an ingestion-time cutover (`auto_since`) so pre-existing history is not unexpectedly reprocessed. A partial turn opened before activation but completed/ingested afterward is eligible. An enabled legacy configuration without a cutover remains fail-closed until the policy is explicitly re-enabled. Discovery cannot see uncaptured ChatGPT history, cannot ingest incomplete/untrusted evidence, and cannot perform semantic synthesis. An empty job queue alone is not evidence that all conversations were captured or distilled.
