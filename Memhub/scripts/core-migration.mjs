...
const repoRoot = resolve(memhubDir, "..");
...
const memorySource = resolve(args["memory-source"] ?? join(repoRoot, "Memory", "dist", "src"));
const memoryVendor = resolve(args["memory-vendor"] ?? join(memhubDir, "vendor", "memory-core", "src"));
const agentSourceCore = resolve(args["agent-source-core"] ?? join(repoRoot, "AgentSourceCore", "dist", "src", "index.js"));
const agentSourceVendor = resolve(args["agent-source-vendor"] ?? join(memhubDir, "vendor", "agent-source-core", "index.js"));
const normifySource = resolve(args["normify-source"] ?? join(repoRoot, "..", "Normify", "lib"));
...
async function preflight() {
  ...
  const memoryParity = await compareTrees("memory-core", memorySource, memoryVendor, normalizeMemoryCore);
  const agentSourceParity = await compareFiles("agent-source-core", agentSourceCore, agentSourceVendor);
  const normifyParity = await compareTrees("architecture-core", normifySource, normifyVendor);
  if (!memoryParity.equal || !agentSourceParity.equal || !normifyParity.equal) {
    fail("vendored core differs from the currently deployed source runtime", {
      memoryParity,
      agentSourceParity,
      normifyParity
    });
...
async function compareFiles(label, sourcePath, vendorPath) {
  requireFile(sourcePath, `${label} source`);
  requireFile(vendorPath, `${label} vendor`);
  const sourceText = (await readFile(sourcePath, "utf8")).replaceAll("\r\n", "\n").trimEnd();
  const vendorText = (await readFile(vendorPath, "utf8")).replaceAll("\r\n", "\n").trimEnd();
  const sourceSha256 = sha256Text(sourceText);
  const vendorSha256 = sha256Text(vendorText);
  return {
    label,
    equal: sourceSha256 === vendorSha256,
    sourcePath,
    vendorPath,
    sourceSha256,
    vendorSha256
...
function normalizeMemoryCore(rel, content) {
  if (rel !== "agent-source/runtime.js") return content;
  return content
    .replace('from "@memmy/agent-source-core"', 'from "__MEMHUB_AGENT_SOURCE_CORE__"')
    .replace('from "../../../agent-source-core/index.js"', 'from "__MEMHUB_AGENT_SOURCE_CORE__"');
}
