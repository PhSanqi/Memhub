...
export function syncMemoryModelCatalog(root, memory, patch) {
    ...
    function syncMemoryRole(role, assignmentKey, capability) {
        ...
        if (presetId)
            assignment[assignmentKey] = presetId;
    }
}
function upsertMemoryCatalogPreset(root, connection, capability) {
    ...
    if (!endpointId) {
        endpointId = uniqueCatalogId(`memmy-memory-${shortHash(`${providerId}\0${protocol}\0${apiBase}\0${apiKey ?? ""}\0${stableJson(extraHeaders)}\0${stableJson(extraBody)}`)}`, endpoints);
    ...
    if (!presetId) {
        presetId = uniqueCatalogId(`memmy-memory-${shortHash(`${providerId}\0${endpointId}\0${model}`)}`, presets);
        presets[presetId] = {
            provider: providerId,
            endpoint: endpointId,
            model,
        ...
        const capabilities = Array.isArray(preset.capabilities)
            ? preset.capabilities.filter((value) => typeof value === "string")
            : [];
...
function catalogProviderId(connection) {
...
function uniqueCatalogId(base, values) {
    if (!(base in values))
        return base;
    let suffix = 2;
    while (`${base}-${suffix}` in values)
        suffix += 1;
    return `${base}-${suffix}`;
}
