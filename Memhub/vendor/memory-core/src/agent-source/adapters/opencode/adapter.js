...
import { readOpencodeDatabase, streamOpencodeDatabase } from "./db-reader.js";
const OPENCODE_SOURCE_ID = "opencode";
/** Creates create opencode source adapter. */
export function createOpencodeSourceAdapter(deps = {}) {
    ...
    return {
        ...
        async *scan(options) {
            ...
            for (const [targetIndex, target] of targets.entries()) {
                ...
                for await (const rawMessage of streamConversationWindow(options.fullHistory ? streamOpencodeDatabase(target.databasePath) : readOpencodeDatabase(target.databasePath), options.since, options.signal, remainingMessageCapacity(options.maxMessages, emittedMessages), options.fullHistory)) {
                    throwIfAborted(options.signal);
                    options.onProgress?.({
                        sourceId: descriptor.sourceId,
                        phase: "redact",
...
async function discoverOpencodeTargets(databasePath) {
