...
/** Rollout reader module. */
export async function* readCodexRollout(filePath, signal) {
...
function toRawCodexMessage(record, rolloutId, lineNumber, toolNamesByCallId) {
...
function toToolMessage(payload, rolloutId, lineNumber, createdAt, toolNamesByCallId) {
        ...
        if (callId) {
            ...
            if (toolNamesByCallId.size > MAX_TOOL_NAME_ENTRIES) {
                ...
                if (typeof oldest === "string")
                    toolNamesByCallId.delete(oldest);
            }
        }
        return {
            messageId: `${rolloutId}:${lineNumber}`,
 ...
 */
function parseRolloutId(filePath) {
