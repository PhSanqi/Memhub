...
export async function* readWorkbuddyHistory(filePath, signal) {
...
export function extractWorkbuddyUserMessage(record, fallbackConversationId, lineNumber) {
    const message = extractWorkbuddyMessage(record, fallbackConversationId, lineNumber);
    return message?.role === "user" ? message : null;
}
export function extractWorkbuddyMessage(record, fallbackConversationId, lineNumber) {
    return toRawWorkbuddyMessage(record, fallbackConversationId, lineNumber);
}
function toRawWorkbuddyMessage(record, fallbackConversationId, lineNumber) {
    ...
    const conversationId = firstString(record.sessionId, record.conversationId, record.threadId, nestedMessage?.sessionId, nestedMessage?.conversationId) ?? fallbackConversationId;
...
function extractMessageText(record, nestedMessage, role) {
...
function extractText(value, depth = 0) {
    ...
    for (const key of ["text", "content", "message", "output", "result", "value"]) {
...
function stripWorkbuddySystemXmlTags(value) {
    let text = value
