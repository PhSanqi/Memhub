...
export function classifyUserMemory(text) {
    const content = text.trim();
    if (!content || isUserMemoryQuestion(content) || isQuestionOnly(content) || DYNAMIC_CURRENT_FACT_PATTERN.test(content))
        return [];
    const types = [];
    ...
    if (FACT_PATTERNS.some((pattern) => pattern.test(content)))
        types.push("User Fact");
    return [...new Set(types)];
}
export function isUserMemoryQuestion(text) {
...
export function normalizeUserMemoryText(text) {
    return text
        .normalize("NFKC")
        .toLocaleLowerCase()
...
export function userMemoryTextHash(text) {
...
export function isPureUserMemoryStatement(text) {
