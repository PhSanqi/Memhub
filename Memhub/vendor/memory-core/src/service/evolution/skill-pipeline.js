import { SKILL_CRYSTALLIZE_PROMPT, SKILL_REBUILD_PROMPT, buildSkillDraft, cosine, detectDominantLanguage, extractToolNamesFromTraces, languageSteeringLine, policyIsEligibleForDownstream, policyMetaFromMemory, skillEtaAfterRewardDrift, skillMetaFromMemory, skillStatusAfterRewardDrift, traceMetaFromMemory, verifySkillDraft } from "../../algorithm/plugin-algorithms.js";
...
import { profileIdFromMemory, projectIdFromMemory, sameProjectScope } from "../namespace/namespace-scope.js";
...
export class SkillPipeline {
    deps;
    skillCrystallizationRuns = new Map();
    constructor(deps) {
        this.deps = deps;
    }
    invalidatePolicySource(policyId, at) {
        const affected = this.deps.repos.memories
            ...
            .filter((item) => Boolean(item.skill?.sourcePolicyIds.includes(policyId) &&
        ...
        for (const { memory, skill } of affected) {
            const activePolicies = skill.sourcePolicyIds
                ...
                .filter((policy) => Boolean(policy && policyIsEligibleForDownstream(policy)));
            ...
            const steps = originalSteps.flatMap((step) => {
                ...
                if (supportingPolicyIds.length === 0) {
                    if (step.required === true || step.core === true)
                        removedRequiredStep = true;
                    return [];
                }
            ...
            const policyContentHashes = filterRecordByKeys(isRecord(internalSkill.policy_content_hashes)
    ...
    async crystallizeSkill(job) {
        ...
        for (const policyMemory of policyMemories) {
            ...
            if (!policy || !policyIsEligibleForDownstream(policy))
                continue;
            ...
            const existingSkill = this.consolidateCompatibleSkillsForPolicy(policy, at, targetSkillId, job.payload.reason !== "policy.skill_fanout");
            ...
            const verifiedDraft = {
                ...
                procedureJson: attachSkillStepPolicySources(draft.procedureJson, existingSkill?.memory, policy.id)
            };
            ...
            for (const episodeId of uniq(evidenceTraces.map((trace) => trace.episodeId).filter((id) => Boolean(id)))) {
            ...
            recordApiLog(this.deps.repos.runtime, upsert.created ? "skill_generate" : "skill_evolve", { phase: "done", skillId: upsert.memory.id, policyId: policy.id }, {
            ...
            }, elapsedApiLogMs(startedAt), true, nowIso());
    ...
    consolidateCompatibleSkillsForPolicy(policy, at, targetSkillId, allowTargetMerge = true) {
        ...
        const mergeable = seed
            ? mutableSkills.filter((skill) => skill.id !== seed.id &&
        ...
        const policyContentHashes = Object.assign({}, ...candidates.map((skill) => storedSkillPolicyContentHashes(skill.memory)));
    ...
    isSkillCrystallizationInCooldown(policy, at) {
    ...
    markSkillCrystallizationRun(policy, at) {
        if (this.deps.config.algorithm.skill.cooldownMs <= 0)
            return;
        const now = Date.parse(at);
        if (!Number.isFinite(now))
            return;
    ...
    skillCrystallizationCooldownKey(policy) {
        return policy.id;
    }
    gatherSkillEvidence(policy) {
        const byId = new Map();
        const episodeIds = new Set(policy.sourceEpisodeIds);
        const failureEpisodeIds = new Set(policy.sourceEpisodeIds.filter((episodeId) => this.isFailureEpisodeForSkillEvidence(episodeId)));
        if (episodeIds.size > 0) {
            const candidates = this.deps.repos.memories
                ...
                .filter((trace) => Boolean(trace?.episodeId &&
        ...
        for (const memory of this.deps.repos.memories.getMany(policy.sourceTraceIds)) {
            ...
            if (trace && (!trace.episodeId || !failureEpisodeIds.has(trace.episodeId)))
                byId.set(trace.id, trace);
        }
        const traces = Array.from(byId.values())
            .filter((trace) => trace.userText !== "[REDACTED]" &&
    ...
    isFailureEpisodeForSkillEvidence(episodeId) {
    ...
    gatherSkillCounterExamples(policy) {
    ...
    applySkillRewardDriftForPolicy(policy, at) {
        ...
        for (const skill of skills) {
            ...
            if (eta === skill.eta && status === skill.status)
                continue;
            ...
            recordApiLog(this.deps.repos.runtime, "skill_evolve", { phase: "done", skillId: saved.id, policyId: policy.id, reason: "reward_drift" }, {
                skillId: saved.id,
    ...
    async enhanceSkillDraft(policy, fallback, evidenceTraces, counterExamples, existingSkill) {
        ...
        try {
            ...
            const outputLanguage = skillOutputLanguageFor(this.deps.config.algorithm.skill.outputLanguageMode, detectDominantLanguage(languageSamples));
            ...
            const procedureJson = rebuild
                ? mergeSkillRebuildProcedureJson(skillProcedureJsonFromMemory(existingSkill?.memory), llmProcedureJson, rebuild.level, stringArray(result.changed_sections ?? result.changedSections))
                : llmProcedureJson;
...
function updateSkillStats(memory, input) {
...
function memoryStatusForSkillStatus(status) {
...
function capSkillPromptText(value, max) {
    if (value.length <= max)
        return value;
...
function filterRecordByKeys(value, keys) {
    return Object.fromEntries(Object.entries(value).filter(([key]) => keys.has(key)));
}
function coerceSkillProcedureJson(result) {
    ...
    return {
        ...
        steps,
        examples,
        decisionGuidance: coerceSkillDecisionGuidance(result.decisionGuidance ??
            result.decision_guidance),
        tags: dedupeCaseInsensitiveStrings(tags),
        tools: dedupeCaseInsensitiveStrings(tools)
    };
}
function skillOutputLanguageFor(mode, detected) {
    if (mode === "zh" || mode === "en")
        return mode;
    return detected === "zh" ? "zh" : "en";
}
function skillEvidenceEpisode(runtime, episodeId) {
    return episodeId ? runtime.getEpisode(episodeId) : undefined;
}
function skillEvidenceEpisodeOutcome(episode) {
...
function skillRebuildPlan(policy, existingSkill, evidenceTraces) {
    ...
    const level = previousPolicyHash !== policyHash
        ? "L2"
...
function skillPolicyContentHash(policy) {
    return stableHash({
        title: policy.title,
        trigger: policy.trigger,
        procedure: policy.procedure,
...
function storedSkillPolicyContentHash(memory, policyId) {
    const internal = memory.properties.internal_info;
    ...
    const hashes = isRecord(skill.policy_content_hashes)
        ...
        : isRecord(internal.policy_content_hashes)
            ? internal.policy_content_hashes
            : {};
    if (policyId && typeof hashes[policyId] === "string")
        return hashes[policyId];
    const value = skill.policy_content_hash ?? internal.policy_content_hash;
    return typeof value === "string" && value ? value : null;
}
function storedSkillPolicyContentHashes(memory) {
    if (!memory)
        return {};
    const internal = memory.properties.internal_info;
    const skill = isRecord(internal.skill) ? internal.skill : {};
    const raw = isRecord(skill.policy_content_hashes)
        ...
        : isRecord(internal.policy_content_hashes)
            ? internal.policy_content_hashes
            : {};
    return Object.fromEntries(Object.entries(raw).filter((entry) => typeof entry[1] === "string"));
}
function skillProcedureJsonFromMemory(memory) {
    if (!memory)
        return {};
    const internal = memory.properties.internal_info;
    const skill = isRecord(internal.skill) ? internal.skill : {};
    const fromSkill = skill.procedure_json;
    if (isRecord(fromSkill))
        return fromSkill;
    const fromInternal = internal.procedure_json;
    return isRecord(fromInternal) ? fromInternal : {};
}
function mergeSkillRebuildProcedureJson(existing, draft, level, changedSections) {
    if (Object.keys(existing).length === 0) {
        return draft;
    }
    const allowed = skillRebuildAllowedSections(level, changedSections);
    const mergeField = (field) => allowed.has(field) ? draft[field] ?? existing[field] : existing[field] ?? draft[field];
    return {
...
function mergeCompatibleSkillProcedures(memories) {
...
function procedureWithPolicySources(memory) {
    const procedure = skillProcedureJsonFromMemory(memory);
...
function mergeCompatibleSkillSteps(existing, incoming) {
    if (!Array.isArray(existing) || !Array.isArray(incoming))
        return existing ?? incoming;
    const merged = existing.filter(isRecord).map((step) => ({ ...step }));
    const byId = new Map(merged.flatMap((step) => typeof step.id === "string" && step.id ? [[step.id, step]] : []));
...
function mergeSkillSteps(existing, draft) {
    if (!Array.isArray(existing) || !Array.isArray(draft))
        return draft ?? existing;
    const additions = draft.filter(isRecord);
    const byId = new Map(additions.flatMap((step) => typeof step.id === "string" && step.id ? [[step.id, step]] : []));
    ...
    for (const step of additions)
        if (!consumed.has(step))
            merged.push(step);
    return merged;
}
function attachSkillStepPolicySources(procedure, existingMemory, policyId) {
    ...
    const existingSteps = Array.isArray(existingProcedure.steps)
        ...
        : [];
    const byId = new Map(existingSteps.flatMap((step) => typeof step.id === "string" && step.id ? [[step.id, step]] : []));
...
function normalizeSkillStepTitle(value) {
    return skillText(value).toLowerCase().replace(/[\s_-]+/g, " ").trim();
}
function isReadOnlySkillMemory(memory) {
    const internal = memory.properties.internal_info;
    return internal.read_only === true || internal.generated_by_memory_base !== true;
}
function compareSkillMergeTargets(left, right) {
    const active = (skill) => skill.status === "active" ? 1 : 0;
...
function skillPolicyCompatibility(skill, policy) {
    if (skill.memory.userId !== policy.memory.userId)
        return 0;
    const skillProject = projectIdFromMemory(skill.memory);
    const policyProject = projectIdFromMemory(policy.memory);
    // Account-level skills and project-level skills are different knowledge
    // products. Never let a global skill absorb a project policy (or vice
    // versa), and never merge policies across projects.
    if (!sameProjectScope(skillProject, policyProject))
        return 0;
    const skillProfile = profileIdFromMemory(skill.memory);
    const policyProfile = profileIdFromMemory(policy.memory);
    if (skillProfile && policyProfile && skillProfile !== policyProfile)
        return 0;
    ...
    const lexical = textTokenOverlap(`${skill.name}\n${skill.invocationGuide}`, `${policy.title}\n${policy.trigger}\n${policy.procedure}`);
    if (!tagCompatible && lexical < 0.25)
        return 0;
    const vector = skill.vec && policy.vec ? cosine(skill.vec, policy.vec) : 0;
    return Math.max(lexical, vector);
}
function skillsAreCompatibleForMerge(left, right) {
    if (left.memory.userId !== right.memory.userId)
        return false;
    const leftProject = projectIdFromMemory(left.memory);
    const rightProject = projectIdFromMemory(right.memory);
    if (!sameProjectScope(leftProject, rightProject))
        return false;
    ...
    if (![...leftTags].some((tag) => rightTags.has(tag)))
        return false;
    const lexical = textTokenOverlap(`${left.name}\n${left.invocationGuide}`, `${right.name}\n${right.invocationGuide}`);
...
function textTokenOverlap(left, right) {
    const tokens = (value) => new Set(value.toLowerCase().match(/[\p{Script=Han}]{2,}|[a-z0-9_]{3,}/gu) ?? []);
    const a = tokens(left);
...
function stableSkillKey(draft, policyMemory) {
...
function skillRebuildAllowedSections(level, changedSections) {
    if (level === "L0") {
        return new Set(["retrievalBlurb", "summary"]);
    }
    const normalized = new Set(changedSections.map((section) => section.trim().toLowerCase().replace(/_/g, "")));
    if (normalized.size === 0) {
        return new Set(["retrievalBlurb", "summary", "decisionGuidance", "steps"]);
    }
    const mapped = new Set();
    if (normalized.has("retrievalblurb"))
        mapped.add("retrievalBlurb");
    if (normalized.has("triggercontext"))
        mapped.add("triggerContext");
    if (normalized.has("summary"))
        mapped.add("summary");
    if (normalized.has("parameters"))
        mapped.add("parameters");
    if (normalized.has("preconditions"))
        mapped.add("preconditions");
    if (normalized.has("steps"))
        mapped.add("steps");
    if (normalized.has("examples"))
        mapped.add("examples");
    if (normalized.has("decisionguidance"))
        mapped.add("decisionGuidance");
    if (normalized.has("tags"))
        mapped.add("tags");
    if (normalized.has("tools"))
        mapped.add("tools");
    return mapped.size > 0 ? mapped : new Set(["retrievalBlurb", "summary", "decisionGuidance", "steps"]);
}
function skillStepsFromProcedure(procedure) {
...
function skillCrystallizerInvalidReason(result) {
    if (!isRecord(result))
        return "llm-failed: skill.crystallize.invalid: non-object output";
    ...
    if (!skillText(result.summary))
        return "llm-failed: skill.crystallize.invalid: missing summary";
    if (coerceSkillSteps(result.steps).length === 0) {
        return "llm-failed: skill.crystallize.invalid: missing steps";
    }
    return null;
}
function detectSkillModelRefusal(value) {
    for (const text of collectSkillStrings(value)) {
        if (detectSkillModelRefusalText(text))
            return true;
    }
    return false;
}
function detectSkillModelRefusalText(text) {
...
function collectSkillStrings(value) {
    if (typeof value === "string")
        return [value];
    if (Array.isArray(value))
        return value.flatMap(collectSkillStrings);
    if (!value || typeof value !== "object")
        return [];
    return Object.values(value).flatMap(collectSkillStrings);
}
function coerceSkillSteps(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map((item) => {
...
function coerceSkillParameters(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map((item) => {
...
function coerceSkillExamples(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map((item) => {
...
function coerceSkillDecisionGuidance(value) {
    if (!isRecord(value))
...
function skillTextArray(value) {
    if (!Array.isArray(value))
        return [];
    return value.map((item) => skillText(item)).filter(Boolean);
}
function skillMarkdownArray(value) {
    if (!Array.isArray(value))
        return [];
...
function stripDangerousMarkdownLinks(text) {
    return text.replace(SKILL_MARKDOWN_LINK_RE, (_match, bang, label, rawUrl) => {
...
function coerceSkillName(value, fallback) {
...
function dedupeCaseInsensitiveStrings(values) {
    const out = [];
    const seen = new Set();
