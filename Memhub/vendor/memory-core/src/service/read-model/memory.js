...
export function detailFromMemory(memory, processing) {
...
export function detailTitleForMemory(memory) {
...
export function detailSummaryForMemory(memory) {
    const trace = traceMetaFromMemory(memory);
    const policy = policyMetaFromMemory(memory);
    const worldModel = worldModelMetaFromMemory(memory);
    const skill = skillMetaFromMemory(memory);
...
export function firstDetailDisplayString(...values) {
    return values.map(cleanDetailDisplayText).find((value) => Boolean(value && !isWorldSectionHeadingForDisplay(value) && !isInternalMemoryKeyForDisplay(value)));
}
export function memoryDetailWithLayerPayload(detail, memory) {
    const item = { ...detail };
    if (memory.memoryLayer === "L1") {
        ...
        const policy = policyMetaFromMemory(memory);
        item.policy = { utilityScore: policy?.gain, confidence: policy?.confidence, evidenceMemoryIds: policy?.sourceTraceIds ?? sourceMemoryIdsFromMemory(memory), repairHints: policy?.verification ? [policy.verification] : [] };
    }
...
export function memoryEtag(memory) { return `${memory.id}-v${memory.version}`; }
export function sourceMemoryIdsFromMemory(memory) { return stringArrayFromInternal(memory, "source_memory_ids").concat(stringArrayFromInternal(memory, "source_l1_memory_ids")).concat(stringArrayFromInternal(memory, "source_policy_ids")).concat(stringArrayFromInternal(memory, "evidence_anchor_ids")); }
export function procedureFromSkillMemory(memory) { const value = memory.properties.internal_info.procedure_json ?? memory.properties.internal_info.procedure; if (Array.isArray(value))
    return value.filter((item) => typeof item === "string"); if (typeof value === "string") {
    try {
...
function firstReadableDetailMemoryLine(value) { return value.split(/\r?\n/).map(cleanDetailDisplayText).find((line) => Boolean(line && !isWorldSectionHeadingForDisplay(line) && !isInternalMemoryKeyForDisplay(line))); }
function cleanDetailDisplayText(value) { const text = (value ?? "").replace(/^\s*Summary:\s*/i, "").replace(/^\s*#{1,6}\s+/, "").replace(/^\s*[-*]\s+/, "").replace(/\*\*([^*]+)\*\*/g, "$1").trim(); return text || undefined; }
function isWorldSectionHeadingForDisplay(value) { return /^(Environment|Inference|Constraints|Environment Knowledge|环境|环境拓扑|行为规律|约束禁忌|结构化认知)$/i.test(value.trim()); }
function isInternalMemoryKeyForDisplay(value) { return Boolean(value && /^(trace|policy|world|world_model|skill)[:_]/i.test(value.trim())); }
export function truncateDetailTitle(value) { return value.length <= 80 ? value : `${value.slice(0, 77)}...`; }
function stringArrayFromInternal(memory, key) { return stringArray(memory.properties.internal_info[key]); }
...
function stringFromMaybeRecord(record, key) { return isRecord(record) ? stringFromRecord(record, key) : undefined; }
function stringFromRecord(record, key) { const value = record[key]; return typeof value === "string" ? value : undefined; }
function rawTurnIdFromMemory(memory) { const source = memory.properties.internal_info.source_raw_turn_id; if (typeof source === "string" && source)
    return source; const raw = memory.properties.internal_info.raw_turn_id; if (typeof raw === "string" && raw)
    return raw; const trace = memory.properties.internal_info.trace; return isRecord(trace) ? stringFromRecord(trace, "raw_turn_id") : undefined; }
