...
- memory: account/project memory access over the existing Memory Core.
...
The architecture adapter uses the embedded engine by default; the old Normify
CLI is compatibility-only. EmbeddedMemoryCore can launch the vendored headless
...
`vendor/memory-core` launcher and the gateway requires that unit. Legacy
`memmy-memory.service` and the external Normify CLI are rollback/compatibility
surfaces rather than production dependencies.
...
The memhub architecture tree already reserves memhub.core for this work.
