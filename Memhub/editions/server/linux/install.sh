...
"$NODE" "$REPO_ROOT/Memhub/dist/mcp.js" account list --state-root "$SERVER_STATE" > "$TMP"
...
if [[ -z "$ACCOUNT_ID" ]]; then
  if [[ -n "$EMAIL" ]]; then
    "$NODE" "$REPO_ROOT/Memhub/dist/mcp.js" account add "$USERNAME" "$EMAIL" --state-root "$SERVER_STATE" > "$TMP"
  else
    "$NODE" "$REPO_ROOT/Memhub/dist/mcp.js" account add "$USERNAME" --state-root "$SERVER_STATE" > "$TMP"
  fi
