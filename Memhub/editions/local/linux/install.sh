...
ensure_build() {
  if [[ -f "$REPO_ROOT/Memory/dist/src/server/index.js" && -f "$REPO_ROOT/Memhub/dist/mcp.js" && -f "$REPO_ROOT/Memhub/dist/bridge.js" ]]; then
    return
  fi
...
"$NODE" "$REPO_ROOT/Memhub/dist/mcp.js" account list --state-root "$SERVER_STATE" > "$ACCOUNTS_JSON"
...
if [[ -z "$ACCOUNT_ID" ]]; then
  "$NODE" "$REPO_ROOT/Memhub/dist/mcp.js" account add local --state-root "$SERVER_STATE" > "$ACCOUNTS_JSON"
  ACCOUNT_ID="$("$NODE" - "$ACCOUNTS_JSON" <<'NODE'
...
if [[ ! -f "$BRIDGE_JSON" ]] || ! "$NODE" -e 'const fs=require("node:fs");const p=process.argv[1];try{const x=JSON.parse(fs.readFileSync(p,"utf8"));process.exit(x.device_token?0:1)}catch{process.exit(1)}' "$BRIDGE_JSON"; then
  ...
  "$NODE" "$REPO_ROOT/Memhub/dist/mcp.js" device add "$ACCOUNT_ID" "local-$(hostname)" --state-root "$SERVER_STATE" > "$DEVICE_TMP"
...
NODE
  ...
  MEMHUB_BRIDGE_HOME="$STATE_ROOT" MEMHUB_DEVICE_TOKEN="$DEVICE_TOKEN" \
    "$NODE" "$REPO_ROOT/Memhub/dist/bridge.js" configure \
...
After=memhub-memory.service
Requires=memhub-memory.service

[Service]
Type=simple
EnvironmentFile=$ENV_PATH
ExecStart=$NODE $REPO_ROOT/Memhub/dist/mcp.js --http 3001 --http-path /memhub/mcp --capture-path /memhub/capture --state-root $SERVER_STATE --memory-url http://127.0.0.1:18960
Restart=on-failure
RestartSec=3s
UMask=0077
...
ExecStart=$NODE $REPO_ROOT/Memhub/dist/bridge.js serve --port 17861
Restart=on-failure
RestartSec=3s
UMask=0077
...
systemctl --user enable --now memhub-memory.service memhub-local.service memhub-bridge.service
