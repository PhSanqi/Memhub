...
$MemoryEntry = Join-Path $RepoRoot "Memory\dist\src\server\index.js"
$McpEntry = Join-Path $RepoRoot "Memhub\dist\mcp.js"
$BridgeEntry = Join-Path $RepoRoot "Memhub\dist\bridge.js"
if (-not $SkipBuild -and (!(Test-Path $MemoryEntry) -or !(Test-Path $McpEntry) -or !(Test-Path $BridgeEntry))) {
  Push-Location $RepoRoot
  try {
    & $Npm install --legacy-peer-deps
    if ($LASTEXITCODE -ne 0) { throw "npm install failed" }
    & $Npm run 'build:headless' -w '@memmy/memory'
    if ($LASTEXITCODE -ne 0) { throw "Memory build failed" }
    & $Npm --prefix Memhub run build --workspaces=false
